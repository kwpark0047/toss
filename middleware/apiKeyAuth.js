...
const apiKeyAuth = async (req, res, next) => {
  ...
  if (!key) {
    return res
      ...
      .json({
        error: 'unauthorized',
        message: 'API 키가 필요합니다. X-API-Key 헤더를 확인하세요.',
      });
  }
  try {
    const hash = hashApiKey(key);
    const rows = await prisma.$queryRawUnsafe(
      'SELECT id, store_id, scopes, revoked FROM api_keys WHERE key_hash = $1 LIMIT 1',
      hash
    );
    const rec = rows[0];
    if (!rec || rec.revoked) {
      return res
        .status(401)
        .json({ error: 'unauthorized', message: '유효하지 않거나 폐기된 API 키입니다.' });
    }
    req.apiClient = {
      keyId: rec.id,
      storeId: rec.store_id,
      scopes: String(rec.scopes || 'read')
        .split(',')
        .map((s) => s.trim()),
    };
    // last_used_at 갱신은 실패해도 요청 흐름 유지
    prisma
      .$executeRawUnsafe('UPDATE api_keys SET last_used_at = NOW() WHERE id = $1', rec.id)
...
/** 스코프 요구 (기능별 scope를 우선하고 기존 read/write도 하위호환) */
const requireScope = (scope) => (req, res, next) => {
  const scopes = req.apiClient?.scopes || [];
  const [area, action] = scope.split(':');
  const allowed =
    scopes.includes(scope) ||
    (action === 'read' && scopes.includes('read')) ||
    (action === 'write' && scopes.includes('write')) ||
    (area === 'print' && scopes.includes('write'));
  if (!allowed) {
    return res
      .status(403)
      .json({
        error: 'insufficient_scope',
        message: `이 작업에는 '${scope}' 스코프가 필요합니다.`,
      });
  }
  next();
};
