 ...
 *
 * [변경 이력 — H-6]
 * 과거에는 `xss-clean` 패키지를 사용했으나
 *   1) 2021년 이후 유지보수가 중단되었고,
 *   2) Express 5 에서 `req.query` 가 read-only getter 로 바뀌면서 예외를 던져
 *      결국 "쿼리스트링 살균을 통째로 건너뛰는" 상태로 방치되어 있었다.
 * 이제 의존성을 제거하고, Express 5 에서도 동작하는 자체 구현으로 대체한다.
 * (query 는 Object.defineProperty 로 살균본을 다시 정의한다)
 *
 * 방어 계층 위치: 입력 살균은 "심층 방어"일 뿐이며, 최종 방어선은 출력 인코딩과
 * CSP(cspNonceMiddleware)이다.
 */
...
const sanitizeString = (val) => {
  ...
  const rules = [
    ...
    // 닫히지 않은 script/iframe/object/embed 여는 태그
 ...
 */
const sanitizeDeep = (input, depth = 0, counter = { n: 0 }) => {
 ...
 */
const redefineQuery = (req, sanitized) => {
  try {
 ...
 */
const basicXssProtection = (req, res, next) => next();
