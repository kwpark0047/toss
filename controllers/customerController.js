...
const customerController = {
  ...
  async phoneJoin(req, res) {
    const { phone, store_id, order_id } = req.body;
    if (!phone || !store_id) {
      return res
        .status(400)
        .json({ success: false, message: '휴대폰 번호와 매장 ID는 필수입니다.' });
    }
    if (!order_id) {
      return res.status(400).json({ success: false, message: '주문 정보가 필요합니다.' });
    }
    // capability: 서버가 검증한 주문 컨텍스트 — 금액은 클라이언트 입력이 아닌 DB 주문으로 산정
  ...
  async issueCoupon(req, res) {
    ...
    const result = await customerService.issueCoupon(
      req.params.storeId,
    ...
    res.json({
      success: true,
      data: result.issued,
      message: `${result.couponName} 쿠폰이 발급되었습니다.`,
    });
  },

  // GET /detail/:customerId — 특정 단골고객 상세 조회
  async getDetail(req, res) {
    const result = await customerService.getCustomerDetail(
      req.params.customerId,
      req.user.id,
      req.user.role
    );
  ...
  async registerFcmToken(req, res) {
    ...
    if (!phone || !store_id || !fcm_token) {
      return res
        .status(400)
        .json({ success: false, message: 'phone, store_id, fcm_token은 필수입니다.' });
    }
