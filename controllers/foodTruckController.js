...
const foodTruckController = {
  ...
  toggleSession: catchAsync(async (req, res) => {
    ...
    res.success(
      truck,
      is_active_session ? '영업 세션이 시작되었습니다.' : '영업 세션이 마감되었습니다.'
    );
  }),
  ...
  ingredientSoldOut: catchAsync(async (req, res) => {
    ...
    res.success(
      result,
      `재료 소진으로 관련 상품 ${result.updatedCount}개가 일시 품절 처리되었습니다.`
    );
  }),
  ...
  processOfflineSync: catchAsync(async (req, res) => {
    ...
    res.success(
      result,
      `성공적으로 ${result.synchronizedCount}건의 오프라인 결제 데이터를 안전하게 동기화하였습니다.`
    );
  }),

  /**
   * [GET] 사업자 트럭 디자인 쇼케이스 콘셉트 조회
   */
  getDesign: catchAsync(async (req, res) => {
    const { storeId } = req.params;
    const designTheme = await FoodTruckService.getDesignTheme(storeId);
    res.success(
      { store_id: parseInt(storeId), design_theme: designTheme },
      '적용 중인 디자인 쇼케이스 콘셉트를 불러왔습니다.'
    );
  }),

  /**
   * [PUT] 사업자 트럭 디자인 쇼케이스 콘셉트 저장
   */
  updateDesign: catchAsync(async (req, res) => {
    const { storeId } = req.params;
    const { design_theme } = req.body;

    if (!design_theme) {
      throw new AppError('design_theme 필드는 필수입니다.', 400);
    }

    const truck = await FoodTruckService.updateDesignTheme(storeId, design_theme);
    res.success(
      { store_id: parseInt(storeId), design_theme: truck.design_theme },
      '디자인 쇼케이스 콘셉트가 저장되었습니다.'
    );
  }),
  ...
  getAnalytics: catchAsync(async (req, res) => {
    ...
    res.success(
      analytics,
      '푸드트럭의 지능형 피크타임 및 거점별 매출 분석 보고서가 성공적으로 로드되었습니다.'
    );
  }),
};
