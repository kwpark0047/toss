...
const vars = [
  ...
  { key: 'GEMINI_API_KEY', level: levels.OPTIONAL, desc: 'Gemini AI (→ AI 기능 비활성화)' },
  { key: 'NCP_GEOCODE_KEY_ID', level: levels.OPTIONAL, desc: 'NCP 지오코딩 대안' },
  { key: 'NCP_GEOCODE_KEY', level: levels.OPTIONAL, desc: 'NCP 지오코딩 대안' },
  { key: 'CORS_ORIGIN', level: levels.OPTIONAL, desc: 'CORS 허용 오리진 (기본 http://localhost:3000)' },
...
function checkEnv() {
  ...
  if (missingRequired.length > 0) {
    const msg = `[WARN] 설정되지 않은 환경변수 (관련 기능 동작 안 함):\n${missingRequired.map(v => `  - ${v.key} (${v.desc})`).join('\n')}`;
    console.warn(msg);
    warnings.push(msg);
  }
