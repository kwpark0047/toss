/**
 * i18nTranslations.js — 알림톡/영수증/알림 다국어 번역 사전 (ko/en/ja/zh)
 *
 * 사용 예:
 ...
 *
 * 번역 키 구조:
 *   alimtalk.{TEMPLATE_CODE}         — 카카로 알림톡 템플릿
...
const translations = {
  ...
  en: {
    alimtalk: {
      ...
      FOOD_READY: {
        title: 'Food Ready',
        message: 'Order {{orderNumber}} (Table {{tableName}}) at {{storeName}} is ready.',
      },
      ORDER_CANCELLED: {
 ...
 */
function t(key, lang = 'ko', vars = {}) {
  const langDict = translations[lang] || translations.ko;
 ...
 */
function getAlimtalkTemplate(templateCode, lang = 'ko', vars = {}) {
 ...
 * @returns {{title: string, message: string}}
 */
function getNotificationTemplate(type, lang = 'ko', vars = {}) {
