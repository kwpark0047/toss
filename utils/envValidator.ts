...
const vars: { key: string; level: Level; desc: string }[] = [
  // CRITICAL - 서버 실행 필수
 ...
 */
export function checkEnv(): { ok: boolean; warnings: string[] } {
  const missingCritical: Array<{ key: string; desc: string }> = [];
  const missingRequired: Array<{ key: string; desc: string }> = [];
  const missingOptional: Array<{ key: string; desc: string }> = [];
