...
const encryptWithIvKey = (normalized: string, ivKey: string): string => {
 ...
 */
export const decryptPhone = (stored: string): string => {
 ...
 * DB의 암호화된 값과 비교 가능
 */
export const encryptPhoneForSearch = (phone: string): string => {
 ...
 */
export const phoneSearchCandidates = (phone: string): string[] => {
    const normalized = normalizePhone(phone);
    if (!normalized) return [];
    const candidates = [normalizePhone(phone)];
    if (ENC_KEY && HMAC_KEY) {
        candidates.unshift(encryptWithIvKey(normalizePhone(phone), LEGACY_HMAC_KEY));
        candidates.unshift(encryptWithIvKey(normalizePhone(phone), HMAC_KEY));
    }
 ...
 */
export const decryptPhoneFields = (obj: any, fields: string[] = ['phone', 'customer_phone']): any => {
    if (!obj) return obj;
    const result = { ...obj };
