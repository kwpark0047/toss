  ...
  product: {
    create: Joi.object({
      ...
      options: Joi.alternatives()
        .try(Joi.array().items(Joi.object().unknown(true)).allow(null), Joi.string().allow('', null))
        .optional(),
    }).unknown(true),
  },
      ...
      options: Joi.string()
        .custom((value, helpers) => {
          try {
            ...
            const { error } = Joi.array().items(Joi.object().unknown(true)).min(1).validate(parsed);
            if (error)
              return helpers.error('any.invalid', { message: '옵션 형식이 올바르지 않습니다.' });
            return value;
          } catch {
  ...
  order: {
    create: Joi.object({
      ...
      payment_method: Joi.string()
        .valid('cash', 'card', 'transfer', 'toss', 'naver', 'kakao')
        .required(),
      items: Joi.array()
        .items(
          Joi.object({
