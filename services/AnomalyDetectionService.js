    ...
    async checkSalesAnomaly(storeId, io = null) {
        try {
            ...
            pastOrders.forEach(order => {
                const hoursDiff = Math.floor((now.getTime() - new Date(order.created_at).getTime()) / (60 * 60 * 1000)) - 1;
            ...
            logger.info(`[Anomaly Engine] Store ${numericStoreId} | Current Slot: ${currentHourOrders} | Historical Mean: ${mean.toFixed(2)} | StdDev: ${stdDev.toFixed(2)} | Z-Score: ${zScore.toFixed(2)}`);
            ...
            if (Math.abs(zScore) >= 2.0 && (currentHourOrders >= 5 || mean >= 2)) {
                ...
                if (io) {
                    ...
                    logger.info(`[Anomaly Socket] Successfully broadcasted system:anomaly_alert to Store ${numericStoreId}`);
                }
