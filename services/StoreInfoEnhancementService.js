...
class StoreInfoEnhancementService {
  ...
  calculateCompletionScore(store) {
    ...
    return {
      ...
      filledFields: Object.values(fields)
        .flat()
        .filter((f) => this.getNestedValue(store, f)).length,
  ...
  // AI 기반 매장 정보 자동 보완
  async enhanceStoreInfo(storeId) {
    ...
    if (!store.vat_rate) enhancements.vat_rate = 0.1;
...
고객의 방문 욕구를 자극하는 톤앤매너로 작성해주세요.
  ...
  async getRecommendations(store) {
    ...
    if (!store.plan || store.plan === 'free') {
      recs.push({
        type: 'plan',
        message: '프로/엔터프라이즈 플랜 업그레이드로 고급 기능 활용 가능',
      });
    }
