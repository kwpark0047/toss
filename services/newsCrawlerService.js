...
async function saveNews(articles, sourceName) {
  ...
  for (const article of articles) {
    ...
    try {
      ...
      dbLogger.error(
        { error: error.message, link: article.link },
        `Failed to save news from ${sourceName}`
      );
    }
  }
      ...
      [
        {
          ...
          category: '공지사항',
          summary:
            '2024년 소상공인 정책자금 지원 계획을 안내해 드립니다. 자세한 내용은 첨부파일을 확인해주세요.',
...
async function crawlAllSources() {
  ...
  try {
    await Promise.allSettled([crawlSemas(), crawlMss(), crawlKfme(), crawlKfa()]);
