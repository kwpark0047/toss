...
class CustomerService {
  ...
  // capability: { orderId, storeId } — 서버가 검증한 주문 컨텍스트.
  ...
  async phoneJoin(input, capability = null) {
    ...
    // 중복 적립 방지 + 모든 변이는 트랜잭션 안에서 원자적으로
    const txResult = await prisma.$transaction(async (tx) => {
      const alreadyEarned = await tx.point_transactions.findFirst({
      ...
      const earnPoints =
        amount > 0 ? await Point.calculateEarnPoints(amount, storeId, { phone: rawPhone }) : 0;

      let pointResult = null;
      let totalPoints = 0;
      if (earnPoints > 0) {
        const points = await tx.user_points.findFirst({
          where: { phone: { in: phoneSearchCandidates(rawPhone) }, store_id: storeId },
        });
        const currentBalance = points?.total_points || 0;
        const balanceAfter = currentBalance + earnPoints;
        pointResult = await tx.point_transactions.create({
          data: {
            user_point_id: points?.id,
            store_id: storeId,
            order_id: orderId,
            type: 'earn',
            amount: earnPoints,
            balance_after: balanceAfter,
            reference_id: `order:${orderId}:loyalty-earn`,
            description: `주문 적립 (${amount.toLocaleString()}원)`,
          },
        });
        totalPoints = balanceAfter;
      } else {
        const balance = await tx.user_points.findFirst({
          where: { phone: { in: phoneSearchCandidates(rawPhone) }, store_id: storeId },
        });
    ...
    if (txResult.duplicate) return txResult;

    const { customer, earnPoints, totalPoints, newTier, welcomeCoupon, nextTier } = txResult;

    // 알림 등록 (부수 작업 — 실패해도 로열티 적립은 유지)
    await prisma.notifications
      .create({
        data: {
          ...
          type: 'NEW_ORDER',
          title: '주문 알림 등록',
          message: `${rawPhone} 고객님의 알림이 등록되었습니다.`,
          priority: 'low',
        },
      })
    ...
    return {
      ...
      next_tier: nextTier
        ? { name: nextTier.tier_name, remaining: nextTier.min_spent - txResult.newTotalSpent }
        : null,
