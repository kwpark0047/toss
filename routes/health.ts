...
router.get('/deep', async (req, res) => {
  ...
  const errorRatePct = Number(checks.sla.errorRatePct);
  if (errorRatePct > 1 || checks.sla.p99Ms > 3000) {
    alerting.send({
      level: 'critical',
      title: 'SLO 임계치 초과',
      message: `error rate ${errorRatePct}% / p99 ${checks.sla.p99Ms}ms`,
      meta: { errorRatePct, p99Ms: checks.sla.p99Ms, req5m: checks.sla.req5m },
    });
  }
  ...
  if (!overallOk) {
    alerting.send({
      level: 'warn',
      title: '헬스체크 이상 감지',
      message: JSON.stringify(checks.database || checks.memory),
      meta: payload,
    });
  }
...
router.get('/sla', (req, res) => {
  ...
  res.json({
    ...
    target: {
      uptimePct: SLO_UPTIME_PCT,
      maxDowntimePerQuarterMin: 648,
      p99MaxMs: 3000,
      errorRateMaxPct: 1.0,
    },
    current: {
