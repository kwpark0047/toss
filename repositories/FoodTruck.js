...
const FoodTruck = {
  ...
  findByStoreId: async (storeId) => {
    try {
      ...
      console.error(
        `[Prisma Error] FoodTruck.findByStoreId failed for store_id: ${storeId}`,
        error
      );
      throw error;
    }
  ...
  toggleSession: async (storeId, isActiveSession) => {
    try {
      ...
      console.error(
        `[Prisma Error] FoodTruck.toggleSession failed for store_id: ${storeId}`,
        error
      );
      throw error;
    }
  ...
  toggleEmergencySoldOut: async (storeId, isEmergencySoldOut) => {
    try {
      ...
      console.error(
        `[Prisma Error] FoodTruck.toggleEmergencySoldOut failed for store_id: ${storeId}`,
        error
      );
      throw error;
    }
  ...
  /**
   * 고객 노출용 디자인 쇼케이스 콘셉트 테마 저장 (Upsert)
   */
  updateDesign: async (storeId, designTheme) => {
    try {
      return await prisma.food_trucks.upsert({
        where: { store_id: parseInt(storeId) },
        update: { design_theme: designTheme },
        create: {
          store_id: parseInt(storeId),
      ...
      console.error(`[Prisma Error] FoodTruck.updateDesign failed for store_id: ${storeId}`, error);
      throw error;
    }
  },
