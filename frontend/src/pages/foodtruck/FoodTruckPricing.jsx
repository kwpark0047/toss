...
import {
  Truck, Menu, X, Check, Sparkles
} from 'lucide-react';

export default function FoodTruckPricing() {
  ...
  const navItems = [
    ...
    { label: '요금제', to: '/foodtruck/landing', active: true },
  ...
  const plans = [
    {
      name: '무료 (Free)',
      price: 0,
      desc: '푸드트럭 기본 운영 및 QR 코드 생성을 지원하는 스타터 요금제',
      features: [
        '테이블별 무제한 QR 코드 생성',
        '기본 모바일 웹 메뉴판 렌더링',
        '실시간 조리 대기열 (기본 KDS)',
    ...
    },
    {
      name: '프로 (Pro)',
      price: 20000,
      desc: '이동식 푸드트럭 운영에 필수적인 실시간 위치 싱크 및 자동 알림 요금제',
      features: [
        'Toss Payments 실제 라이브 결제창 연동',
        '이동식 푸드트럭 실시간 GPS 위치 싱크',
        '하트비트 연동 알림톡 예비 수단 자동 발송',
        '긴급 재료소진 일괄 품절 제어 (킬스위치)',
    ...
    },
    {
      ...
      price: 79000,
      desc: '다대 대형 푸드트럭 fleet 운영 및 AI 기반 거점 최적화 패키지',
      features: [
        '모든 Pro 요금제 기능 전사 보장',
        'AI 기반 요일/시간별 피크타임 매출 동향 예측',
        '주간/월간 Gemini 인공지능 매장 경영 분석 리포트',
        'ESC/POS 주방 무선 프린터 연동 및 인쇄 대기열',
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      {/* Pricing Cards */}
      <section className="pb-24 lg:pb-36 max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          {plans.map((plan, idx) => (
            <div
              key={idx}
              className={`rounded-3xl border p-8 flex flex-col justify-between transition-all relative ${plan.color} ${
                plan.popular ? 'shadow-2xl shadow-orange-500/5 ring-1 ring-orange-500/30' : ''
              }`}
            >
              {plan.popular && (
