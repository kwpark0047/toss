...
function buildTruckIcon(L) {
  return L.divIcon({
    className: '',
    html: `<div style="position:relative;width:34px;height:42px;filter:drop-shadow(0 3px 4px rgba(0,0,0,.3))">` + `<div style="width:34px;height:34px;background:#fff;border:2.5px solid #f97316;border-radius:50% 50% 50% 0;transform:rotate(-45deg);display:flex;align-items:center;justify-content:center;">` + `<span style="transform:rotate(45deg);font-size:16px;line-height:1;">🚚</span></div></div>`,
    iconSize: [34, 42],
    iconAnchor: [17, 40],
    popupAnchor: [0, -38]
  });
...
/* ─── Fetch active trucks ─── */
  ...
  const processedTrucks = useMemo(() => {
    return trucks.map(truck => ({
      ...truck,
      distance: userCoords ? calculateDistance(userCoords.latitude, userCoords.longitude, truck.latitude, truck.longitude) : null
    })).filter(truck => {
      const matchesSearch = truck.store?.name?.toLowerCase().includes(searchTerm.toLowerCase()) || truck.geocoded_address?.toLowerCase().includes(searchTerm.toLowerCase()) || truck.store?.category?.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesCategory = categoryFilter === 'all' || truck.store?.category === categoryFilter;
  ...
  useEffect(() => {
    ...
    processedTrucks.forEach(truck => {
      ...
      const m = L.marker([truck.latitude, truck.longitude], {
        icon: truckIcon,
        title: truck.store?.name
      });
    ...
    if (userCoords) {
      const um = L.marker([userCoords.latitude, userCoords.longitude], {
        icon: userIcon,
        title: '내 위치'
      });
      layer.addLayer(um);
    ...
    if (pts.length === 1) map.setView(pts[0], 15);else if (pts.length > 1) map.fitBounds(pts, {
      padding: [50, 50],
      maxZoom: 15
    });
  ...
  return <div className="min-h-screen bg-white text-slate-900 flex flex-col font-sans">
      ...
      <header className="border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {/* Live indicator */}
            {lastRefresh && <span className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-600 text-[10px] font-black">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> LIVE
              </span>}
          </div>
          <div className="flex items-center gap-2">
            <Link to="/admin" className="text-xs font-bold text-slate-500 hover:text-slate-900 px-3 py-1.5 rounded-lg border border-slate-200 bg-white transition-all">
              점주 센터
            </Link>
          </div>
        </div>
      ...
      <div className="sticky top-16 z-30 border-b border-slate-200 bg-white/90 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex gap-1 overflow-x-auto scrollbar-hide py-2 -mx-1">
            ...
            label: '문의하기',
            icon: Mail
          }].map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-black whitespace-nowrap transition-all ${activeTab === tab.id ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-white text-slate-500 hover:bg-slate-50 hover:text-slate-800 border border-slate-200'}`}>
                <tab.icon size={13} />
                {tab.label}
              </button>)}
          </div>
      ...
      {activeTab === 'finder' && <section className="border-b border-slate-200 bg-white py-12 px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            title: '동선 최적화',
            desc: '연료·매출 동시 계산'
          }].map((feat, i) => <div key={i} className="bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-2xl p-4 text-center transition-all hover:scale-[1.02]">
      ...
      <section className="max-w-6xl mx-auto w-full px-4 pt-10 pb-6 text-center md:text-left md:flex md:items-center md:justify-between gap-8 relative z-10">
        ...
        <div className="flex justify-center md:justify-end">
          <button onClick={handleGetLocation} disabled={locatingUser} className="flex items-center gap-2.5 px-6 py-4 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-600 hover:from-orange-600 hover:to-rose-700 text-white font-bold text-sm shadow-xl shadow-orange-500/15 hover:shadow-orange-500/25 active:scale-95 disabled:opacity-50 transition-all">
      ...
      <section className="max-w-6xl mx-auto w-full px-4 mb-6 space-y-4 relative z-10">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="푸드트럭 명칭, 요리, 상권(홍대, 강남역 등) 검색..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 text-sm font-bold focus:outline-none focus:border-orange-500/50 transition-all focus:ring-4 focus:ring-orange-500/5" />
        </div>

        {/* Category chips */}
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {CATEGORIES.map(cat => <button key={cat.id} onClick={() => setCategoryFilter(cat.id)} className={`shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition-all border ${categoryFilter === cat.id ? 'bg-orange-500 border-orange-500 text-white shadow-lg shadow-orange-500/15' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-800'}`}>
              {cat.label}
            </button>)}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto w-full px-4 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10 pb-12">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex items-center justify-between px-1 mb-2">
            ...
            {userCoords && <span className="text-xs text-orange-500 font-bold">📍 내 위치 연결</span>}
          </div>

          {loading ? <div className="space-y-4">
              {[1, 2, 3].map(i => <div key={i} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 animate-pulse">
            ...
            </div> : error ? <div className="p-8 text-center rounded-2xl border border-slate-200 bg-slate-50 text-slate-500">
            ...
            </div> : processedTrucks.length === 0 ? <div className="p-12 text-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 text-slate-500">
            ...
            return <div key={truck.store_id} onClick={() => setSelectedTruck(truck)} className={`border transition-all rounded-3xl p-5 cursor-pointer flex flex-col sm:flex-row gap-5 relative overflow-hidden group ${isSelected ? 'bg-orange-50 border-orange-500 ring-2 ring-orange-500/10' : 'bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                  {/* Status badges */}
                  <div className="absolute top-4 right-4 flex gap-1.5">
                    {isEmergency && <span className="px-2 py-0.5 rounded-md bg-rose-500/15 border border-rose-500/25 text-rose-500 text-[10px] font-bold flex items-center gap-1 animate-pulse">
                        <ShieldAlert size={10} /> 품절
                      </span>}
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-black ${isActive ? 'bg-emerald-50 border border-emerald-200 text-emerald-600' : 'bg-slate-100 border border-slate-200 text-slate-500'}`}>
                      {isActive ? '● 영업중' : '준비중'}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 space-y-2 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {truck.store?.category && <span className="text-[10px] font-bold text-orange-600 bg-orange-50 border border-orange-200 px-1.5 py-0.5 rounded-md">
                    ...
                    <div className="flex items-center gap-4 text-xs font-bold pt-1">
                      {isActive && <div className="flex items-center gap-1 text-emerald-600">
                          <Clock size={12} />
                          <span>영업중</span>
                        </div>}
                      {truck.distance != null && <div className="flex items-center gap-1 text-orange-500">
                          <Navigation size={12} />
                ...
                }} disabled={!isActive || isEmergency} className={`w-full sm:w-28 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-black transition-all active:scale-95 ${isActive && !isEmergency ? 'bg-orange-500 hover:bg-orange-600 text-white shadow-lg shadow-orange-500/10' : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'}`}>
                      <ShoppingBag size={12} />
                      주문
                    </button>
                  </div>
        ...
        <div className="lg:col-span-5 h-[400px] lg:h-[calc(100vh-12rem)] lg:sticky lg:top-28 rounded-3xl overflow-hidden border border-slate-200 bg-slate-50 relative">
          ...
          {/* Map loading overlay */}
          {!mapReady && <div className="absolute inset-0 flex items-center justify-center bg-white/80 z-10">
              <p className="text-sm text-slate-500 font-bold">지도를 불러오는 중…</p>
            </div>}
          {/* Selected truck detail panel */}
          {selectedTruck && <div className="absolute bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur-md border-t border-slate-200 z-[500] space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-md shrink-0 ml-2 ${selectedTruck.is_active_session ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>
            ...
            }} disabled={!selectedTruck.is_active_session || selectedTruck.is_sold_out_emergency} className={`w-full py-3 rounded-2xl text-xs font-black flex items-center justify-center gap-1.5 transition-all active:scale-95 ${selectedTruck.is_active_session && !selectedTruck.is_sold_out_emergency ? 'bg-gradient-to-r from-orange-500 to-rose-600 hover:from-orange-600 hover:to-rose-700 text-white shadow-xl shadow-orange-500/10' : 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200'}`}>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-8">
            {[{
              num: '①',
              title: '내 위치 연결',
              desc: 'GPS로 현재 위치를 파악하고 가장 가까운 푸드트럭을 찾습니다.'
            }, {
              num: '②',
              title: '푸드트럭 선택',
              desc: '실시간 영업 상태와 거리를 확인하고 마음에 드는 트럭을 고릅니다.'
            }, {
              num: '③',
              title: '메뉴 주문',
              desc: '메뉴판에서 원하는 음식을 선택하고 바로 주문합니다.'
            }].map((step, i) => <div key={i} className="text-center p-6 rounded-2xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition-colors">
      ...
      {activeTab === 'features' && <div className="relative z-10 flex-1">
      ...
      {activeTab === 'pricing' && <div className="relative z-10 flex-1">
          <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /></div>}>
            <PricingPage />
          </Suspense>
        </div>}

      {activeTab === 'guides' && <div className="relative z-10 flex-1">
          <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /></div>}>
            <GuidesPage />
          </Suspense>
        </div>}

      {activeTab === 'contact' && <div className="relative z-10 flex-1">
          <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /></div>}>
            <ContactPage />
          </Suspense>
        </div>}
