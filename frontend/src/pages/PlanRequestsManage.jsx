...
const PlanRequestsManage = () => {
    ...
    const fetchRequests = useCallback(async () => {
        setLoading(true);
        try {
            const res = await planRequestsAPI.getAll(filter === 'all' ? null : filter);
    ...
    const handleApprove = async (id) => {
        setProcessingId(id);
        try {
            await planRequestsAPI.approve(id, adminNote);
            fetchRequests();
    ...
    const handleReject = async (id) => {
        setProcessingId(id);
        try {
            await planRequestsAPI.reject(id, adminNote);
            fetchRequests();
            setSelectedRequest(null);
            setAdminNote('');
        } catch (err) {
    ...
    const getPlanBadge = (plan) => {
        switch (plan) {
            case 'enterprise':
                return <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full font-medium">{plan.toUpperCase()}</span>;
            case 'pro':
                return <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full font-medium">{plan.toUpperCase()}</span>;
            default:
                return <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full font-medium">{plan.toUpperCase()}</span>;
        }
    };
                        ...
                        >
                            <div className="p-6">
                                ...
                                <div className="flex items-center gap-4 mb-4">
                                    {getPlanBadge(req.current_plan)}
                                    <span className="text-gray-400">→</span>
                                    {getPlanBadge(req.requested_plan)}
                                </div>
...
export default PlanRequestsManage;
