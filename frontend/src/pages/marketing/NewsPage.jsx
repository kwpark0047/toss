...
export default function NewsPage() {
  ...
  const navItems = [
    ...
    { label: '요금안내', path: '/pricing' },
    { label: '가이드', path: '/guides' },
    { label: '푸드트럭', path: '/foodtruck' },
    { label: '뉴스/소식', path: '/news' }
  ];
  ...
  const getSourceColor = (source) => {
    switch (source) {
      case '소상공인시장진흥공단': return 'bg-blue-50 text-blue-600 border-blue-200';
      case '중소벤처기업부': return 'bg-emerald-50 text-emerald-600 border-emerald-200';
      case '소상공인연합회': return 'bg-orange-50 text-orange-600 border-orange-200';
      case '한국프랜차이즈산업협회': return 'bg-purple-50 text-purple-600 border-purple-200';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 mb-8 sticky top-20 z-30">
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
              ...
              <button
                onClick={() => refetch()}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-slate-50 border border-slate-200 rounded-lg hover:bg-slate-100 transition-colors shrink-0"
              >
