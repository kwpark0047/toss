...
export default function PricingPage() {
  ...
  const plans = [{
    ...
    features: ['테이블별 무제한 QR 코드 생성', '기본 모바일 웹 메뉴판 렌더링', '실시간 조리 대기열 (기본 KDS)', '기본 일일 매출 통계 조회', '직원 권한 격리 (Staff 1명)'],
    cta: '무료로 시작',
    ...
    features: ['Toss Payments 실제 라이브 결제창 연동', '실시간 매장 위치 공유 서비스', '고객 알림톡 발송 연동', '긴급 품절 처리 기능', '단골 고객 찜 목록 및 타임세일 푸시 마케팅', '직원/매니저 무제한 계정 생성 및 권한 격리'],
    cta: '프로 시작하기',
    ...
    features: ['모든 Pro 요금제 기능 전사 보장', 'AI 기반 요일/시간별 피크타임 매출 동향 예측', '주간/월간 Gemini 인공지능 매장 경영 분석 리포트', 'ESC/POS 주방 무선 프린터 연동 및 인쇄 잡 대기열', '전용 1:1 슬랙/전화 고객 지원 엔지니어 배정', '99.9% 가동률 보증 보장 협약서(SLA) 체결'],
    cta: '영업문의 및 컨설팅',
    color: 'border-slate-900 bg-slate-950/40',
    popular: false
  }];
  ...
  return <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <section className="relative py-16 sm:py-20 lg:py-28 overflow-hidden text-center">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <button onClick={() => setAnnualBilling(!annualBilling)} className="w-12 h-6 rounded-full bg-slate-200 border border-slate-300 p-1 flex items-center relative transition-all">
      ...
      <section className="pb-20 sm:pb-24 lg:pb-36 max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-stretch">
          {plans.map((plan, idx) => <div key={idx} className={`rounded-3xl border p-6 sm:p-8 flex flex-col justify-between transition-all relative ${plan.popular ? 'bg-white border-orange-200 shadow-xl shadow-orange-500/5 ring-1 ring-orange-200' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
              {plan.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-gradient-to-r from-orange-500 to-rose-500 text-white rounded-full text-[10px] font-black tracking-widest uppercase">
                  POPULAR CHOICE
                </span>}
              ...
              <div>
                ...
                <ul className="space-y-3.5 mb-8">
                  {plan.features.map((feat, fidx) => <li key={fidx} className="flex items-start gap-2.5 text-xs text-slate-600">
              ...
              <Link to={plan.price === 79000 ? '/contact' : '/register'} className={`w-full py-3.5 rounded-2xl text-center text-xs font-black transition-all ${plan.popular ? 'bg-gradient-to-r from-orange-500 to-rose-500 text-white shadow-lg shadow-orange-500/10 hover:shadow-orange-500/20 active:scale-95' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200'}`}>
                {plan.cta}
              </Link>
            </div>)}
        </div>
