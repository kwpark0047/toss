      ...
      {showCart && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end">
          <div className="bg-white w-full rounded-t-[2.5rem] max-h-[90vh] overflow-y-auto shadow-2xl" style={themeStyles}>
            ...
            {orderStep === "confirm" && (<><div className="p-4 space-y-4"><h3 className="font-bold" style={{ color: theme.textColor }}>{t('order.order_info')} ({t('order.optional')})</h3><input type="text" placeholder={t('order.name')} value={orderForm.customer_name} onChange={(e) => setOrderForm({ ...orderForm, customer_name: e.target.value })} className="w-full px-4 py-3 rounded-xl outline-none" style={{ backgroundColor: theme.backgroundColor }} /><input type="tel" placeholder={t('order.phone')} value={orderForm.customer_phone} onChange={(e) => setOrderForm({ ...orderForm, customer_phone: e.target.value })} className="w-full px-4 py-3 rounded-xl outline-none" style={{ backgroundColor: theme.backgroundColor }} /><textarea placeholder={t('order.notes')} value={orderForm.notes} onChange={(e) => setOrderForm({ ...orderForm, notes: e.target.value })} rows={2} className="w-full px-4 py-3 rounded-xl outline-none resize-none" style={{ backgroundColor: theme.backgroundColor }} /><div className="rounded-xl p-4" style={{ backgroundColor: theme.backgroundColor }}><h4 className="font-medium mb-3" style={{ color: theme.textColor }}>{t('order.summary')}</h4>{cart.map(i => (<div key={i.product_id} className="flex justify-between text-sm py-1"><span style={{ color: theme.textColor + "90" }}>{i.product_name} x {i.quantity}</span><span style={{ color: theme.textColor }}>{formatPrice(i.price * i.quantity)}</span></div>))}<div className="flex justify-between font-bold pt-3 mt-3 border-t" style={{ borderColor: theme.primaryColor + "20" }}><span style={{ color: theme.textColor }}>{t('order.total_amount')}</span><span style={{ color: theme.primaryColor }}>{formatPrice(getTotalAmount())}</span></div><div className="flex items-center gap-2 mt-3 text-sm" style={{ color: theme.textColor + "70" }}>{(() => { const m = paymentMethods.find(x => x.id === orderForm.payment_method); return m ? <><m.icon size={16} />{t(`order.methods.${m.id}`)}</> : null; })()}</div></div></div><div className="p-4 border-t space-y-3"><button onClick={openPaymentSheet} className="w-full py-4 text-white rounded-2xl flex items-center justify-center gap-2 font-medium shadow-lg" style={{ background: gradientBg }}><Star size={20} />{formatPrice(getTotalAmount())} {t('order.integrated_payment')}</button><button onClick={handleOrder} className="w-full py-3 rounded-2xl flex items-center justify-center gap-2 font-medium border-2" style={{ borderColor: theme.primaryColor, color: theme.primaryColor }}><Send size={18} />{t('order.standard_payment')}</button></div></>)}
          </div></div>)}
