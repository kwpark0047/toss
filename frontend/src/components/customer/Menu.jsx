...
const Menu = ({
  ...
  const {
    ...
    processQueue,
    pendingCount
  } = useOfflineSync(effectiveStoreId);
  ...
  const fetchStoreData = useCallback(async storeId => {
    try {
      const [storeRes, categoriesRes, productsRes] = await Promise.all([storesAPI.getById(storeId), categoriesAPI.getByStore(storeId), productsAPI.getByStore(storeId)]);
    ...
    const handleOrderUpdated = data => {
      if (data?.order_id === orderSuccess.id) {
        ...
        if (data.status === 'ready' && Notification?.permission === 'granted') {
          new Notification('식사가 준비되었습니다!', {
            body: data.status_label || '음식이 준비되었습니다.'
          });
        }
      }
    };
    const handleCartItemUpdated = data => {
      setCart(prev => {
        if (data.action === 'add' || data.action === 'update') {
          ...
          if (exists) return prev.map(i => (i.product_id || i.id) === data.item.product_id ? {
            ...i,
    ...
    if (table?.id) {
      ...
      const loadCart = async () => {
        try {
          ...
          console.error('공유 장바구니 로드 실패:', err);
        }
      };
      loadCart();
  ...
  const syncCartToServer = async (pid, qty, act) => {
    if (!table?.id) return;
    try {
      const res = await cartAPI.updateItem(table.id, {
        product_id: pid,
        quantity: qty,
        user_phone: userPhone
      });
      if (res.success) {
        ordersAPI.getSocket().emit('update-shared-cart', {
          tableId: table.id,
          item: res.data,
          userPhone,
          action: act
  ...
  const addToCart = product => {
    ...
    setCart(prev => {
      ...
      if (existing) return prev.map(item => (item.product_id || item.id) === product.id ? {
        ...item,
        quantity: newQty
      } : item);
      return [...prev, {
  ...
  const updateQuantity = (pid, d) => {
    setCart(prev => {
      ...
      return prev.map(i => (i.product_id || i.id) === pid ? {
        ...i,
        quantity: newQty
      } : i);
    });
...
<div className="w-20 h-20 rounded-full bg-red-50 flex items-center justify-center mb-6 text-red-500">
  ...
  if (orderSuccess) {
    ...
    return <motion.div initial={{
      ...
      opacity: 1,
      scale: 1
    }} className="min-h-screen flex items-center justify-center p-4" style={{
      background: theme.backgroundColor,
      ...themeStyles
    }}>
...
<motion.div initial={{
          ...
          <div className="mb-8 p-6 rounded-[2rem] text-center shadow-inner" style={{
          backgroundColor: theme.primaryColor + "08"
        }}>
            {orderSuccess.queue_number && <div className="mb-4">
                <p className="text-xs font-black uppercase tracking-widest text-gray-400 mb-1">Queue Number</p>
                <motion.p initial={{
              y: 10
            }} animate={{
              y: 0
            }} className="text-6xl font-black" style={{
...
<Icon icon="Timer" size="md" style={{
          ...
          backgroundColor: theme.backgroundColor
            ...
            <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-200">
              ...
              color: theme.primaryColor
            }}>#{orderSuccess.order_number}</span>
            </div>
            ...
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="font-medium px-2 py-1 rounded-lg text-sm" style={{
              backgroundColor: orderSuccess.status === 'completed' ? '#10b98120' : orderSuccess.status === 'preparing' ? '#8b5cf620' : theme.primaryColor + '20',
              color: orderSuccess.status === 'completed' ? '#10b981' : orderSuccess.status === 'preparing' ? '#8b5cf6' : theme.primaryColor
            }}>
            ...
            <div className="flex justify-between items-center pt-3 border-t border-gray-200">
              ...
              color: theme.primaryColor
            }}>{formatPrice(orderSuccess.total_amount)}</span>
            </div>
          </div>
          ...
          backgroundColor: theme.primaryColor + "10"
            ...
            <div>
              ...
              color: theme.textColor
            }}>{t(`status_msg.${orderSuccess.status}`)}</p>
  ...
  return <div className="min-h-screen pb-28 selection:bg-orange-100" style={{
    background: pageBg,
    ...themeStyles
  }}>
      ...
      />
       ...
       <div className="px-5 pb-6">
         <div className="relative w-full aspect-[21/9] rounded-3xl overflow-hidden shadow-2xl shadow-slate-200/50">
           <AnimatePresence>
            ...
            return <motion.div key={banner.id || idx} initial={{
              opacity: 0,
              x: 100
            }} animate={{
              opacity: 1,
              ...
              duration: 0.5
                   ...
                   {isCustom ? <img src={typeof banner === 'string' ? banner : banner.image_url} alt={banner.title || '배너'} className="w-full h-full object-cover" /> : <>
           ...
           <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5">
             {banners.map((_, idx) => <button key={idx} onClick={() => setCurrentBanner(idx)} className={`w-2 h-2 rounded-full transition-all ${idx === currentBanner ? 'w-6 bg-white' : 'bg-white/30'}`} />)}
           </div>
         </div>
       </div>

       {/* 업종별 인기상품 섹션 - m.fooddream.kr 스타일 */}
       {bestProducts.length > 0 && <div className="px-5 pb-6">
          ...
          backgroundColor: theme.primaryColor + "15"
            ...
            color: theme.primaryColor
             ...
             <h3 className="font-black text-lg" style={{
          color: theme.textColor
        }}>베스트 메뉴</h3>
           </div>
           <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
             {bestProducts.slice(0, 5).map(p => <motion.div key={p.id} whileTap={{
          scale: 0.98
                 ...
                 <div className="p-2">
                   <p className="font-bold text-xs line-clamp-1" style={{
              color: theme.textColor
            }}>{p.name}</p>
                   <p className="text-xs font-black mt-1" style={{
              color: theme.primaryColor
       ...
       <MenuProductList filteredProducts={filteredProducts} theme={theme} translatedDescriptions={translatedDescriptions} addToCart={addToCart} setSelectedStoryProduct={setSelectedStoryProduct} setShowStoryModal={setShowStoryModal} gradientBg={gradientBg} />

      {/* 직원 호출 플로팅 버튼 */}
      <FloatingCallButton onClick={() => setShowCallSheet(true)} primaryColor={theme.primaryColor} />

      {/* 장바구니 바텀 시트 */}
      <AnimatePresence>
        {cart.length > 0 && !showCart && <motion.div initial={{
        y: 100,
        opacity: 0
      }} animate={{
        y: 0,
        ...
        opacity: 0
          ...
          setShowCart(true);
          setOrderStep("cart");
        }} className="w-full py-5 text-white rounded-3xl flex items-center justify-between px-8 shadow-2xl shadow-orange-500/40 font-black text-lg overflow-hidden group" style={{
          background: gradientBg
        }}>
      ...
      {showCart && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end">
          <div className="bg-white w-full rounded-t-[2.5rem] max-h-[90vh] overflow-y-auto shadow-2xl" style={themeStyles}>
            ...
            {orderStep === "cart" && <>
                <div className="p-6 space-y-4">
                  {cart.map(i => <motion.div initial={{
              opacity: 0,
              x: -10
            }} animate={{
              opacity: 1,
              x: 0
            }} key={i.product_id} className="flex items-center justify-between p-4 rounded-3xl bg-slate-50 border border-slate-100">
                      <div>
                        <p className="font-bold text-slate-800">{i.product_name}</p>
                        <p className="text-sm font-black" style={{
                  color: theme.primaryColor
                }}>{formatPrice(i.price)}</p>
                      </div>
                ...
                <AnimatePresence>
                  {dessertRecommendations.length > 0 && <motion.div initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              y: 0
                    ...
                    scale: 0.98
                  }} onClick={() => addToCart(rec)} className="min-w-[200px] bg-white p-3 rounded-2xl shadow-sm flex flex-col gap-2 cursor-pointer border border-white hover:border-orange-200 transition-all">
                ...
                color: theme.primaryColor
                  ...
                  <button onClick={() => setOrderStep("payment")} className="w-full py-5 text-white rounded-[2rem] font-black text-lg shadow-xl shadow-orange-500/30 active:scale-[0.98] transition-all" style={{
              background: gradientBg
            }}>
                    {t('menu.order_now')}
                  </button>
            ...
            {orderStep === "payment" && <>
                <div className="p-6">
                  <h3 className={`text-xl font-black mb-6 ${kioskMode ? 'text-2xl' : ''}`} style={{
              color: theme.textColor
            }}>결제 방식을 선택해주세요</h3>
              ...
              setShowCart(false);
                  ...
                  <div className="space-y-3">
                    {paymentMethods.map(m => <button key={m.id} onClick={() => setOrderForm({
                ...orderForm,
                payment_method: m.id
              })} className={`w-full p-6 rounded-[2rem] border-2 flex items-center gap-5 transition-all ${kioskMode ? 'p-7' : ''}`} style={{
...
<Icon icon="CreditCard" size="lg" style={{
            color: theme.primaryColor
                ...
                <div className="p-8 border-t bg-white/80 backdrop-blur-md sticky bottom-0">
                  <button onClick={() => setOrderStep("confirm")} className={`w-full py-6 text-white rounded-[2rem] font-black text-xl shadow-xl shadow-orange-500/30 active:scale-[0.98] transition-all ${kioskMode ? 'text-2xl py-8' : ''}`} style={{
              background: gradientBg
            }}>
                    {t('order.enter_info')}
                  </button>
            ...
            {orderStep === "confirm" && <>
                <div className="p-6 space-y-5">
                  <h3 className={`text-xl font-black mb-5 ${kioskMode ? 'text-2xl' : ''}`} style={{
              color: theme.textColor
            }}>
                    {t('order.order_info')} ({t('order.optional')})
                  </h3>
                  <input type="text" name="name" autoComplete="name" aria-label={t('order.name')} placeholder={t('order.name')} value={orderForm.customer_name} onChange={e => setOrderForm({
              ...orderForm,
              customer_name: e.target.value
            })} className={`w-full px-6 py-5 rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-orange-400 font-medium border ${kioskMode ? 'text-lg' : 'text-sm'}`} style={{
              backgroundColor: theme.backgroundColor
            }} />
                  <input type="tel" name="tel" autoComplete="tel" inputMode="numeric" aria-label={t('order.phone')} placeholder={t('order.phone')} value={orderForm.customer_phone} onChange={e => setOrderForm({
              ...orderForm,
              customer_phone: e.target.value
            })} className={`w-full px-6 py-5 rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-orange-400 font-medium border ${kioskMode ? 'text-lg' : 'text-sm'}`} style={{
              backgroundColor: theme.backgroundColor
            }} />
                  <textarea aria-label={t('order.notes')} placeholder={t('order.notes')} value={orderForm.notes} onChange={e => setOrderForm({
              ...orderForm,
              notes: e.target.value
            })} rows={3} className={`w-full px-6 py-5 rounded-2xl outline-none resize-none focus-visible:ring-2 focus-visible:ring-orange-400 font-medium border ${kioskMode ? 'text-lg' : 'text-sm'}`} style={{
              backgroundColor: theme.backgroundColor
            }} />
              ...
              backgroundColor: theme.backgroundColor
                    ...
                    {cart.map(i => <div key={i.product_id} className="flex justify-between text-sm py-1">
                        <span style={{
                  color: theme.textColor + "90"
                }}>{i.product_name} x {i.quantity}</span>
                        <span className="font-bold text-slate-800">{formatPrice(i.price * i.quantity)}</span>
                      </div>)}
                    <div className="flex justify-between font-bold pt-3 mt-3 border-t" style={{
                borderColor: theme.primaryColor + "20"
              }}>
                      <span style={{
                  color: theme.textColor
                }}>{t('order.total_amount')}</span>
                      <span style={{
                  color: theme.primaryColor
                }}>{formatPrice(getTotalAmount())}</span>
                    </div>
                ...
                <div className="p-8 border-t bg-white/80 backdrop-blur-md sticky bottom-0">
                  <button onClick={orderForm.payment_method === "card" ? openPaymentSheet : handleOrder} className={`w-full py-6 text-white rounded-[2rem] font-black text-xl shadow-xl shadow-orange-500/30 active:scale-[0.98] transition-all ${kioskMode ? 'text-2xl py-8' : ''}`} style={{
              background: gradientBg
            }}>
      ...
      {/* 통합 결제 시트 */}
      <PaymentSheet isOpen={showPaymentSheet} onClose={() => setShowPaymentSheet(false)} store={store} cart={cart} totalAmount={getTotalAmount()} onPaymentComplete={handlePaymentComplete} userIdentifier={userIdentifier} />

      {/* 함께 결제하기(분할 결제) 시트 */}
      <TogetherPaymentSheet isOpen={showTogetherSheet} onClose={() => setShowTogetherSheet(false)} cart={cart} totalAmount={getTotalAmount()} theme={theme} formatPrice={formatPrice} socket={ordersAPI.getSocket()} tableId={table?.id} onConfirm={splitData => {
      handlePaymentSplit(splitData);
    }} />

      {/* 매니저 호출 시트 및 채팅 드로어 */}
      <ManagerCallSheet isOpen={showCallSheet} onClose={() => setShowCallSheet(false)} store={store} table={table} onOpenChat={() => setShowChatDrawer(true)} onVoiceCall={type => {
      const socket = ordersAPI.getSocket();
      if (socket) {
      ...
      <ChatDrawer isOpen={showChatDrawer} onClose={() => setShowChatDrawer(false)} store={store} table={table} customerInfo={{
      phone: userPhone
    }} />

      {/* 리뷰 작성 모달 연동 */}
      <ReviewModal isOpen={showReviewModal} onClose={() => setShowReviewModal(false)} order={{
      ...orderSuccess,
      store_id: store?.id,
      ...
      {/* 예약 섹션 오버레이 */}
      <AnimatePresence>
        {showReservation && <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[70] flex items-center justify-center p-5">
            <motion.div initial={{
          opacity: 0,
          scale: 0.9
        }} animate={{
      ...
      {/* AI 스토리텔링 모달 */}
      <MenuStoryModal isOpen={showStoryModal} onClose={() => setShowStoryModal(false)} product={selectedStoryProduct} storeName={store?.name} />
    </div>;
};
export default Menu;
