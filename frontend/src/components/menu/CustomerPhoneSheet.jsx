...
const getTierMeta = (t) => ({
  GENERAL: { label: t('points.tier_general'), color: 'text-grey-500', bg: 'bg-grey-100' },
  SILVER:  { label: t('points.tier_silver'), color: 'text-grey-600', bg: 'bg-grey-200' },
  GOLD:    { label: t('points.tier_gold'), color: 'text-yellow-600', bg: 'bg-yellow-100' },
  VIP:     { label: t('points.tier_vip'),  color: 'text-orange-600', bg: 'bg-orange-100' },
  VVIP:   { label: t('points.tier_vvip'), color: 'text-purple-600', bg: 'bg-purple-100' },
});

const getTier = (tierMeta, name) => tierMeta[name] || tierMeta.GENERAL;
    ...
    >
      ...
      <div className="relative px-4 py-3.5 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-bold text-grey-400 uppercase tracking-widest mb-0.5">
            {phase === 'scramble' && t('points.encrypt_scramble')}
            {phase === 'mask'    && t('points.encrypt_mask')}
            {phase === 'seal'    && (sealDone ? t('points.encrypt_seal_done') : t('points.encrypt_seal'))}
          </p>
          <p
      ...
      <div className="px-4 py-2 flex items-center gap-2 bg-white/60">
        ...
        <span className="text-grey-200">|</span>
          <span className="text-[10px] text-grey-400 font-medium">{t('points.pbkdf2_label')}</span>
          <span className="text-grey-200">|</span>
          <span className="text-[10px] text-grey-400 font-medium">{t('points.sha256_label')}</span>
      </div>
    </motion.div>
  );
}
...
export default function CustomerPhoneSheet({
  ...
  const handleSubmit = () => {
    ...
    if (digits.length < 10) {
      setError(t('points.enter_phone_error'));
      return;
    }
    setError('');
  ...
  const handleEncryptDone = useCallback(async () => {
    ...
    try {
      ...
      if (res.already_joined) {
        setError(t('points.error_already'));
        setStep('idle');
        return;
      }
      setResult(res);
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              {estimatedPoints > 0 && (
                <div className="flex items-center gap-3 p-4 bg-orange-50 rounded-2xl border border-orange-100 mb-6">
                  ...
                  <div>
                    <p className="text-xs text-orange-600 font-bold">{t('points.estimated_earn')}</p>
          ...
          {step === 'encrypting' && (
            ...
            >
              ...
              <div className="flex flex-col items-center mb-6">
                ...
                <h3 className="tds-subtitle text-grey-900">{t('points.encrypting_title')}</h3>
                <p className="text-sm text-grey-400 mt-1">{t('points.encrypting_subtitle')}</p>
              </div>
              ...
              <div className="mt-4 space-y-2">
                {[
                  { icon: '🔑', label: t('points.pbkdf2'), desc: t('points.pbkdf2_desc') },
                  { icon: '🛡️', label: t('points.aes256_gcm'),    desc: t('points.aes256_gcm_desc') },
                  { icon: '🔒', label: t('points.hash_store'),        desc: t('points.hash_store_desc') },
                ].map((item, i) => (
                  <motion.div
                    key={item.label}
          ...
          {step === 'loading' && (
            ...
            >
              ...
              <p className="text-grey-500 font-medium">{t('points.registering')}</p>
            </motion.div>
          )}
          ...
          {step === 'done' && result && (
            ...
            >
              ...
              <h2 className="text-center tds-title text-grey-900 mb-1">
                {result.is_new_customer ? t('points.welcome_member') : t('points.earned_complete')}
              </h2>
              <p className="text-center text-sm text-grey-400 mb-6">
                {t('points.become_regular', { store: storeName })}
              </p>
              ...
              {result.points_earned > 0 && (
                ...
                >
                  <p className="text-orange-100 text-sm font-medium mb-1">{t('points.earned_this_order')}</p>
              ...
              <div className="space-y-3 mb-5">
                <div className="flex items-center justify-between py-3 px-4 bg-grey-50 rounded-xl">
                  <span className="text-sm text-grey-500 font-medium">{t('points.cumulative')}</span>
                  <span className="font-black text-grey-900">{AnimatedTotal.toLocaleString()}P</span>
                </div>
                <div className="flex items-center justify-between py-3 px-4 bg-grey-50 rounded-xl">
                  <span className="text-sm text-grey-500 font-medium">{t('points.tier')}</span>
                  <span className={`text-sm font-black px-2.5 py-1 rounded-lg ${getTier(tierMeta, result.customer_tier).bg} ${getTier(tierMeta, result.customer_tier).color}`}>
                    {getTier(tierMeta, result.customer_tier).label}
                  </span>
                </div>
                {result.next_tier && (
                  <div className="flex items-center justify-between py-3 px-4 bg-grey-50 rounded-xl">
                    <span className="text-sm text-grey-500 font-medium">{t('points.next_tier_remaining')}</span>
                    <span className="text-sm font-bold text-orange-500">
                      {t('points.remaining_to_tier', { remaining: result.next_tier.remaining.toLocaleString(), tier: result.next_tier.name })}
                    </span>
                  </div>
                )}
              </div>
              ...
              {result.welcome_coupon && (
                ...
                >
                  ...
                  <div>
                    <p className="text-xs text-purple-500 font-bold">{t('points.first_visit_benefit')}</p>
                    <p className="text-sm font-black text-purple-900">{result.welcome_coupon.name}</p>
                    <p className="text-xs text-purple-400">
                      {result.welcome_coupon.type === 'percent'
                        ? t('points.discount_percent', { amount: result.welcome_coupon.amount })
                        : t('points.discount_amount', { amount: result.welcome_coupon.amount.toLocaleString() })}
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-xl mb-5">
                ...
                <p className="text-xs text-blue-600 font-medium">{t('points.live_notification')}</p>
              </div>
