...
const CategoryTabs = ({ categories, selectedCategory, onSelectCategory }) => {
  ...
  if (useSegmented) {
    return (
      <div className="sticky top-14 z-30 bg-[var(--color-bg,#f8fafc)] px-4 pt-3 pb-2">
          ...
          {categories.map((category) => {
            ...
            return (
              ...
              >
                ...
                <span className={`relative z-10 text-sm font-bold transition-colors ${isActive ? 'text-slate-900' : 'text-slate-500'}`}>
                  {category}
                </span>
              </button>
            );
  ...
  // Tab (언더라인 + 가로 스크롤)
  return (
    <div className="sticky top-14 z-30 bg-white/85 backdrop-blur-md border-b border-slate-100 overflow-x-auto no-scrollbar">
      <div className="px-2 flex items-center gap-0.5 min-w-max h-12">
        {categories.map((category) => {
          ...
          return (
            ...
            >
              <span className={`text-sm transition-colors ${isActive ? 'text-primary font-bold' : 'text-slate-500 font-medium'}`}>
                {category}
              </span>
              {isActive && (
                <motion.div
