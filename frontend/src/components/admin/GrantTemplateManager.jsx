...
export default function GrantTemplateManager({
  ...
  return <motion.div initial={{
    ...
    opacity: 1
      ...
      scale: 1
    }} onClick={e => e.stopPropagation()} className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto shadow-2xl">
        ...
        {message && <div className={`mx-4 mt-3 px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 ${message.ok ? 'bg-emerald-500/15 text-emerald-300' : 'bg-rose-500/15 text-rose-300'}`}>
            {message.ok ? <Check size={14} /> : <X size={14} />} {message.text}
          </div>}

        <div className="p-4 space-y-2">
          {!creating && !editing && <button onClick={startCreate} className="w-full h-10 rounded-xl border border-dashed border-white/20 text-xs font-black text-slate-400 hover:text-white hover:border-orange-500/50 transition-all flex items-center justify-center gap-1.5">
              <Plus size={14} /> 새 템플릿 추가
            </button>}

          {(creating || editing) && <div className="bg-white/5 rounded-xl p-3 space-y-2.5">
              <input value={form.title} onChange={e => setForm(f => ({
            ...f,
            title: e.target.value
          }))} placeholder="템플릿 제목 (예: 신규가입 포인트)" spellCheck={false} className="w-full h-9 px-3 rounded-lg bg-white/5 border border-white/10 text-xs outline-none focus:border-orange-500/50" />
              <textarea value={form.reason} onChange={e => setForm(f => ({
            ...f,
            reason: e.target.value
          }))} placeholder="사유 내용 (예: 신규 가입 축하 포인트)" spellCheck={false} rows={2} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs outline-none focus:border-orange-500/50 resize-none" />
              <label className="flex items-center gap-2 text-[11px] text-slate-400">
                <input type="checkbox" checked={form.is_auto} onChange={e => setForm(f => ({
              ...f,
              ...
              setCreating(false);
              setEditing(null);
            }} className="h-9 px-4 rounded-lg bg-white/5 border border-white/10 text-xs font-black text-slate-400 hover:text-white">취소</button>
              </div>
            </div>}

          {loading ? <div className="space-y-2">{[...Array(3)].map((_, i) => <div key={i} className="skeleton-dark h-12 rounded-xl" />)}</div> : templates.length === 0 ? <p className="text-center py-8 text-slate-500 text-xs font-bold">등록된 템플릿이 없습니다.</p> : templates.map(t => <div key={t.id} className="flex items-center justify-between bg-white/5 rounded-xl px-3 py-2.5">
