...
const AdminChatManager = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (socket && activeRoom) {
      socket.emit('join-chat-room', { roomId: activeRoom.id });
  ...
  }, [socket, activeRoom, isSuperAdmin]);
  ...
  const accessSupportRoom = async (targetUserId = null) => {
    setLoading(true);
    try {
      const res = await chatAPI.accessAdminRoom({ user_id: targetUserId });
      if (res.success) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-end pointer-events-none p-4 sm:p-6">
          <motion.div
              ...
              {isSuperAdmin && !activeRoom ? (
                ...
                <div className="flex flex-col h-full">
                  <div className="p-4">
                    <div className="relative">
                      ...
                      <input 
                        type="text"
                        placeholder="사업자명 또는 메시지 검색..."
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm focus:outline-none focus:border-orange-500/50 transition-colors"
                      />
                    </div>
                  </div>
                        ...
                        {filteredRooms.map(room => (
                          ...
                          >
                            <div className="relative">
                              ...
                              {room.unreadCount > 0 && (
                                <span className="absolute -top-1 -right-1 w-5 h-5 bg-orange-500 text-[10px] font-black text-white flex items-center justify-center rounded-full border-2 border-slate-900 animate-bounce">
                                  {room.unreadCount}
                                </span>
                              )}
                            </div>
                ...
                <div className="flex flex-col h-full relative">
                  {loading && messages.length === 0 ? (
                    <div className="absolute inset-0 z-10 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center text-orange-400">
                  ...
                  <div className="p-4 bg-slate-800/50 border-t border-white/5">
                    <form onSubmit={handleSendMessage} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500/50 transition-all placeholder:text-slate-600"
                      />
                      <button
                        type="submit"
                        disabled={!input.trim()}
                        className="p-3 bg-orange-500 text-white rounded-2xl hover:bg-orange-600 disabled:opacity-50 disabled:hover:bg-orange-500 transition-all shadow-lg shadow-orange-500/20"
                      >
                        <Send size={20} />
                      </button>
                    </form>
