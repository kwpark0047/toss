...
export default function StoreSetupWizard() {
  ...
  const handleSaveStore = async () => {
    ...
    try {
      const apiData = {
        ...storeForm,
        business_type:
          storeForm.business_type === '__custom__'
      ...
      if (store?.alreadyOwned) {
        sayWithDelay(
          `"${store.store?.name || storeForm.name}" 매장은 이미 등록되어 있습니다! 🏪 바로 관리하러 이동할게요!`,
          100,
          true
        );
        setTimeout(
          () => navigate(`/admin/stores/${store.store?.id}/orders`, { replace: true }),
          3000
        );
        return;
      }
    ...
    try {
      ...
      if (p) {
        ...
        newItems[idx] = {
          ...
          price: p.price ? String(p.price) : newItems[idx].price,
        };
  ...
  const getMenuUrl = (table) =>
    table?.qr_code
      ? buildQrUrl(table.qr_code)
      : buildMenuUrl(createdStore?.id, table.table_number || '');
  ...
  const handleFinish = () => {
    ...
    const celebrations = [
      {
        msg: '🎉 완성! 사장님 정말 대단해요! 이제 스마트 매장 운영을 시작해볼까요?',
        delay: 0,
        happy: true,
      },
  ...
  const updateMenuItem = (i, f, v) =>
    setMenuItems((p) => p.map((m, idx) => (idx === i ? { ...m, [f]: v } : m)));
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            style={{ width: '100%', height: '0.3mm', background: '#e2e8f0', margin: '1.5mm 0' }}
          />
          ...
          >
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-10 text-center">
              ...
              <p className="text-slate-400 text-base mb-2">
                저는 AI 도우미 <span className="text-amber-400 font-black">팅커벨</span>이에요!
              </p>
              <p className="text-slate-400 text-sm mb-8">
                지금부터 <strong className="text-white">4단계</strong>로 매장 설정을 도와드릴게요.
                <br />
                어렵지 않으니 따라오세요! 😊
              </p>
          ...
          >
              ...
              {STEPS.map((s, i) => {
                ...
                return (
                  <div key={s.id} className="flex-1 flex items-center gap-1.5 sm:gap-2">
                    ...
                    {i < STEPS.length - 1 && (
                      <div
                        className={`flex-1 h-0.5 rounded-full ${done ? 'bg-emerald-500/40' : 'bg-white/10'}`}
                      />
                    )}
                  </div>
                );
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                ...
                >
                  <h2 className="text-xl font-black text-white mb-4 flex items-center gap-2">
                    <Store size={20} className="text-amber-400" /> 매장 정보 입력
                  </h2>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setStoreForm((p) => ({ ...p, open_time: e.target.value }))
                          }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setStoreForm((p) => ({ ...p, close_time: e.target.value }))
                          }
                          onFocus={() => setStep1Focus('time')}
                          className="w-full px-3 py-2.5 bg-white/10 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50 transition-all"
                        />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setStoreForm((p) => ({ ...p, description: e.target.value }))
                        }
                  ...
                  <div className="flex gap-3 mt-4">
                    <button
                      onClick={() => setStep(0)}
                      className="px-5 py-3 bg-white/5 border border-white/10 text-slate-400 rounded-2xl font-bold text-sm hover:bg-white/10 transition-all flex items-center gap-1.5"
                    >
              ...
              {step === 2 && (
                ...
                >
                  <h2 className="text-xl font-black text-white mb-1 flex items-center gap-2">
                    <ChefHat size={20} className="text-amber-400" /> 메뉴 등록
                  </h2>
                  <div className="flex items-center gap-3 mb-5 px-3 py-2.5 bg-gradient-to-r from-violet-500/[0.08] to-purple-500/[0.05] rounded-2xl border border-violet-500/20">
                    <div className="relative flex-shrink-0 w-[38px] h-[38px]">
                      ...
                      >
                        <motion.div
                          ...
                          style={{
                            background:
                              'radial-gradient(circle, rgba(139,92,246,.5) 0%, transparent 72%)',
                          }}
                        ...
                        <Wing side="right" />
                        <div
                          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[24px] h-[24px] rounded-full flex items-center justify-center"
                          style={{
                            background:
                              'radial-gradient(circle at 34% 28%, #FFF8E7, #F59E0B 62%, #D97706)',
                            boxShadow: '0 2px 10px rgba(245,159,11,.5)',
                          }}
                        >
                    ...
                    <p className="text-[12px] font-bold text-violet-200 leading-relaxed flex-1">
                      메뉴 이름을 입력한 뒤{' '}
                      <span className="text-violet-300 font-black">AI ✨ 버튼</span>을 눌러보세요!
                      설명과 가격을 자동으로 완성해드려요.
                    </p>
                  </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          {menuItems.length > 1 && (
                            <button
                              ...
                              className="p-2 text-rose-400/60 hover:text-rose-400 rounded-xl hover:bg-rose-500/10 transition-all"
                            >
                              <Trash2 size={14} />
                            </button>
                          )}
              ...
              {step === 3 && (
                ...
                >
                  <h2 className="text-xl font-black text-white mb-1 flex items-center gap-2">
                    <LayoutGrid size={20} className="text-amber-400" /> 테이블 설정
                  </h2>
                  <p className="text-xs text-slate-500 mb-4">
                    테이블을 추가하거나 삭제해보세요. QR 주문 시 테이블 번호가 표시돼요.
                  </p>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={`text-sm font-black truncate ${isBase ? 'text-amber-300' : 'text-white'}`}
                              >
                                {table.table_number}
                              </p>
                              <p
                                className={`text-[11px] ${isBase ? 'text-amber-500/80' : 'text-slate-500'}`}
                              >
                                {isBase
                                  ? '매장 공통 QR — 테이블 미지정 주문용'
                                  : `${table.capacity}인석`}
                              </p>
                            </div>
                            {!isBase && (
              ...
              {step === 4 && (
                ...
                >
                  <h2 className="text-xl font-black text-white mb-1 flex items-center gap-2">
                    <QrCode size={20} className="text-amber-400" /> QR 코드 출력
                  </h2>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-emerald-400/80">
                        A4 한 장에 4개씩 인쇄됩니다. 코팅 후 각 테이블에 붙여두세요!
                      </p>
                    </div>
                  </motion.div>

                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {['📄 A4 자동 4분할', '🏪 매장 로고·상호', '📞 전화번호', '📱 스캔 안내문'].map(
                      (t) => (
                        <span
                          key={t}
                          className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-full text-[11px] text-slate-400 font-bold"
                        >
                          {t}
                        </span>
                      )
                  ...
                  <div className="grid grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-1 mb-4">
                    {layoutTables.map((table) => {
                      ...
                      return (
                        <div
                          key={table.id}
                          className="bg-white rounded-2xl border border-slate-100 flex flex-col items-center p-3 gap-1 overflow-hidden"
                        ...
                        >
                          ...
                          <p className="text-[11px] font-black text-slate-800 text-center">
                            {isBase ? '테이블 없이 주문' : table.table_number}
                          </p>
                  ...
                  <div id="qr-print-area">{layoutTables.map((t) => renderPrintCard(t))}</div>
