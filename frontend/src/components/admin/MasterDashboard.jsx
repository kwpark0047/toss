...
import {
  ...
  joinDashboard,
  leaveDashboard,
  onDashboardOrderStatusChanged,
  onDashboardForecastUpdate,
  onConnect,
} from '../../utils/socket';
...
const SalesTrendChart = lazy(() =>
  import('./dashboard/DashboardCharts').then((m) => ({ default: m.SalesTrendChart }))
);
const OrderStatusDonut = lazy(() =>
  import('./dashboard/DashboardCharts').then((m) => ({ default: m.OrderStatusDonut }))
);
const PeakHoursBar = lazy(() =>
  import('./dashboard/DashboardCharts').then((m) => ({ default: m.PeakHoursBar }))
);
const SalesForecastWidget = lazy(() =>
  import('./dashboard/DashboardCharts').then((m) => ({ default: m.SalesForecastWidget }))
);
...
const TopProductsList = ({ storeId }) => {
  ...
  useEffect(() => {
    ...
    analyticsAPI
      .getProducts(storeId, start, end, 5, 'quantity')
      .then((res) => {
            ...
            <div key={item.id || idx}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    className={`w-5 h-5 rounded-md flex items-center justify-center font-black text-[9px] ${idx < 3 ? 'bg-orange-500 text-white' : 'bg-white/10 text-slate-400'}`}
                  >
                    {idx + 1}
                  </span>
...
const RecentReviewsFeed = ({ storeId }) => {
  ...
  const renderStars = (rating) => {
    ...
    return Array.from({ length: 5 }, (_, i) => (
      <span key={i} className={i < r ? 'text-amber-400' : 'text-slate-700'} style={{ fontSize: 9 }}>
        ★
      </span>
    ));
  };
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-2 py-0.5 text-[10px] font-black rounded-md border ${c.cls}`}>
      {c.label}
    </span>
  );
};
...
const MasterDashboard = () => {
  ...
  useSEO({
    ...
    description: selectedStore
      ? `${selectedStore.name}의 매출, 주문, 고객 정보를 관리하세요.`
      : '위마켓 스마트 매장 관리',
  });
  ...
  useEffect(() => {
    const cleanup = onOrderUpdated((payload) => {
      setRecentOrders((prev) =>
        prev.map((o) => (o.id === payload.order_id ? { ...o, status: payload.status } : o))
      );
    });
    return cleanup;
  }, []);
  ...
  useEffect(() => {
    ...
    const offForecast = onDashboardForecastUpdate((payload) => {
    ...
    return (
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[0, 1, 2, 3].map((i) => (
            <Skeleton key={i} dark className="h-24 rounded-2xl" />
          ))}
        </div>
        ...
        <div className="space-y-2">
          {[0, 1, 2, 3].map((i) => (
            <Skeleton key={i} dark className="h-16 rounded-2xl" />
          ))}
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh]">
        <EmptyState
          ...
          description={
            isSuper
              ? '지역 커뮤니티/매장 검색에서 매장을 찾거나, 매장 정보 보강 도구를 사용하세요.'
              : '팅커벨 도우미와 함께 첫 매장을 설정해보세요!'
          }
          action={
            isSuper ? (
              <Button
                variant="gradient"
                size="lg"
                onClick={() => navigate('/admin/enrich-stores')}
                className="px-8"
              >
    ...
    <div className="space-y-4 pb-20 md:pb-6 px-0">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          >
            <Bell
              size={16}
              aria-hidden="true"
              className={pendingCount > 0 ? 'text-orange-400' : 'text-slate-500'}
            />
            {pendingCount > 0 && (
          ...
          <button
            type="button"
            onClick={() =>
              isMultiView ? fetchMultiStoreData() : fetchStoreData(selectedStore?.id)
            }
            aria-label="새로고침"
          ...
          >
            <RefreshCw
              size={15}
              aria-hidden="true"
              className={`text-slate-500 ${refreshing ? 'animate-spin text-orange-400' : ''}`}
            />
          </button>
        </div>
      </div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
              {activeCalls.map((call) => {
                ...
                return (
                  <div
                    key={call.id}
                    className="p-3 bg-slate-950/60 border border-white/5 rounded-xl flex items-center justify-between text-xs gap-3"
                  >
                    <div className="min-w-0 flex-1 text-left">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-[10px] font-mono text-slate-500 font-bold">
                        <span>
                          {new Date(call.created_at).toLocaleTimeString('ko-KR', { hour12: false })}
                        </span>
                        <span>·</span>
                        <span
                          className={
                            parsedData.isStaffConnected ? 'text-emerald-500' : 'text-slate-600'
                          }
                        >
      ...
      <div className="grid grid-cols-2 xs:grid-cols-2 gap-2.5 sm:gap-3 px-1">
        {[
          ...
          <div
            key={title}
            className={`bg-white/5 border ${accent} rounded-2xl p-2.5 sm:p-3 flex items-center gap-2.5 sm:gap-3 relative overflow-hidden`}
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-1">
                ...
                {trend !== undefined && (
                  <div
                    className={`shrink-0 flex items-center text-[8px] sm:text-[9px] font-black px-1 py-0.5 rounded-md gap-0.5 ${trend >= 0 ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'}`}
                  >
      ...
      <div className="px-1">
        ...
        <div className="grid grid-cols-3 xs:grid-cols-4 md:grid-cols-6 gap-2 sm:gap-2.5">
          {quickActions.map(({ label, path, icon: Icon, color, badge }) => (
            ...
            >
              <div
                className={`w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center shadow-lg`}
              >
            ...
            multiViewLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-black text-indigo-400 border border-indigo-500/30 rounded-xl hover:bg-indigo-500/10"
                >
                  다시 시도
                </button>
              </div>
              ...
              multiStoreStats.stores.map((s, idx) => (
                <div
                  key={s.store_id}
                  className="px-4 py-3.5 flex items-center justify-between hover:bg-white/5 transition-all border-b border-white/5 last:border-0"
                >
              ...
              {recentOrders.map((order) => (
                ...
                >
                  <div className="relative flex-shrink-0">
                    <div
                      className={`w-11 h-11 rounded-xl flex items-center justify-center font-black text-[11px] ${order.status === 'pending' ? 'bg-orange-500 text-white' : 'bg-white/10 text-slate-400'}`}
                    >
      ...
      <div className="hidden md:block px-1">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            ...
            >
              <div
                className={`w-10 h-10 rounded-xl bg-gradient-to-br ${tool.grad} flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-105 transition-transform`}
              >
      ...
      {/* ── AI 팅커벨 배너 (데스크톱) ── */}
      <div
        className="hidden md:block rounded-3xl p-7 text-white relative overflow-hidden border border-white/10 mx-1"
        style={{ background: 'linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#0f172a 100%)' }}
      >
        <div
          className="absolute -top-10 -right-10 w-44 h-44 rounded-full opacity-20 blur-2xl"
        ...
        <div className="relative z-10 flex items-center justify-between gap-6">
          ...
          <button
            onClick={() => navigate('/admin/tinkerbell')}
