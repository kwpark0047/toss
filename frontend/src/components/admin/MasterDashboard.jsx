...
import { ordersAPI, analyticsAPI, reviewsAPI, exportAPI, getSocket } from '../../api';
...
const SkeletonDonut = () => <div className="bg-white/5 border border-white/10 rounded-2xl p-3 animate-pulse">
...
import { Store, ShoppingBag, DollarSign, Clock, Plus, BarChart3, Users, TrendingUp, Activity, Zap, ArrowUpRight, ArrowDownRight, Sparkles, Settings, RefreshCw, Bell, QrCode, LayoutGrid, ReceiptText, BadgeCheck, ChefHat, AlertCircle, CalendarDays, Download, FileSpreadsheet, FileText, Loader2, MessageSquareText, Code2, Handshake, X, ShieldAlert } from 'lucide-react';
...
const OrderStatusDonut = lazy(() => import('./dashboard/DashboardCharts').then(m => ({
  default: m.OrderStatusDonut
})));
const PeakHoursBar = lazy(() => import('./dashboard/DashboardCharts').then(m => ({
  default: m.PeakHoursBar
})));
const SalesForecastWidget = lazy(() => import('./dashboard/DashboardCharts').then(m => ({
  default: m.SalesForecastWidget
})));
...
const ExportPanel = ({
  ...
  const buttons = [{
    ...
    fn: () => exportAPI.salesExcel(storeId, startDate, endDate)
  }, {
    label: '주문 내역',
    icon: FileSpreadsheet,
    color: 'from-blue-500 to-cyan-500',
    fn: () => exportAPI.ordersExcel(storeId, startDate, endDate)
  }, {
    label: '고객 리스트',
    icon: FileSpreadsheet,
    ...
    fn: () => exportAPI.menuExcel(storeId, startDate, endDate)
  }, {
    label: 'PDF 보고서',
    icon: FileText,
    color: 'from-rose-500 to-pink-500',
    fn: () => exportAPI.reportPdf(storeId, startDate, endDate)
  }];
  return <div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          <input type="date" aria-label="조회 시작일" value={startDate} onChange={e => setStartDate(e.target.value)} className="flex-1 text-[11px] bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-slate-300 outline-none" />
          <span className="text-slate-600 text-xs">~</span>
          <input type="date" aria-label="조회 종료일" value={endDate} onChange={e => setEndDate(e.target.value)} className="flex-1 text-[11px] bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-slate-300 outline-none" />
        </div>
        <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
          ...
          color,
          fn
        }) => <button key={label} onClick={() => run(label, fn)} disabled={!!loading} className="flex flex-col items-center gap-1.5 p-2.5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all disabled:opacity-50 active:scale-95">
              <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${color} flex items-center justify-center`}>
                {loading === label ? <Loader2 size={13} className="text-white animate-spin" /> : <Icon size={13} className="text-white" />}
              </div>
...
const TopProductsList = ({
  ...
  useEffect(() => {
    ...
    analyticsAPI.getProducts(storeId, start, end, 5, 'quantity').then(res => {
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-3 space-y-3">
        ...
        return <div key={item.id || idx}>
              ...
              <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400 transition-all" style={{
...
const RecentReviewsFeed = ({
  ...
  useEffect(() => {
    if (!storeId) return;
    setLoading(true);
    reviewsAPI.getStoreReviews(storeId).then(res => {
  ...
  const renderStars = rating => {
    ...
    return Array.from({
      length: 5
    }, (_, i) => <span key={i} className={i < r ? 'text-amber-400' : 'text-slate-700'} style={{
      fontSize: 9
    }}>
        ★
      </span>);
  ...
  return <div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl divide-y divide-white/5">
        {data.map((review, idx) => <div key={review.id || idx} className="p-3 first:rounded-t-2xl last:rounded-b-2xl">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] font-bold text-slate-300">
                  {review.author || review.customer_name || '익명'}
                </span>
            ...
            <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed">
              {review.content || review.review || review.comment || ''}
            </p>
          </div>)}
      </div>
    </div>;
...
const StatusBadge = ({
  ...
  const c = cfg[status] ?? {
    label: status || '-',
    cls: 'bg-white/10 text-slate-400 border-white/20'
  };
  return <span className={`px-2 py-0.5 text-[10px] font-black rounded-md border ${c.cls}`}>
      {c.label}
    </span>;
};
const MasterDashboard = () => {
  ...
  const fetchMultiStoreData = useCallback(async () => {
    ...
    try {
      ...
      const res = await analyticsAPI.getMultiStore({
        start_date: start,
        end_date: end
      });
      ...
      setStats({
        ...
        by_status: {
          completed: d?.summary?.total_orders ?? 0
        }
      });
    } catch {
  ...
  useEffect(() => {
    if (isMultiView) fetchMultiStoreData();else if (selectedStore) fetchStoreData(selectedStore.id);
  ...
  useEffect(() => {
    const cleanup = onOrderUpdated(payload => {
      setRecentOrders(prev => prev.map(o => o.id === payload.order_id ? {
        ...o,
  ...
  /* ── 실시간 대시보드 구독 (DashboardBroadcastService 전용 채널) ── */
  // 매장 선택 시 대시보드 룸 가입 → order_status_changed / forecast_updated 실시간 수신
  useEffect(() => {
    if (!selectedStore?.id) {
      leaveDashboard();
      return;
    }
    joinDashboard(selectedStore.id);
    const offOrder = onDashboardOrderStatusChanged(payload => {
      // 대시보드 전용 주문 상태 변경 — 목록 즉시 반영 + 통계 갱신
      setRecentOrders(prev => prev.map(o => o.id === payload.order_id ? {
        ...o,
        status: payload.status,
    ...
    const offForecast = onDashboardForecastUpdate(_payload => {
      // 수요 예측이 서버에서 재생성되었음을 실시간 반영
  ...
  }, [selectedStore?.id, fetchStoreData]);
  ...
  if (stores.length === 0) {
    ...
    return <div className="flex flex-col items-center justify-center min-h-[70vh]">
        <EmptyState tone="dark" icon={isSuper ? '🛰️' : '🏪'} title={isSuper ? '관리할 매장을 찾지 못했습니다' : '등록된 매장이 없습니다'} description={isSuper ? '지역 커뮤니티/매장 검색에서 매장을 찾거나, 매장 정보 보강 도구를 사용하세요.' : '팅커벨 도우미와 함께 첫 매장을 설정해보세요!'} action={isSuper ? <Button variant="gradient" size="lg" onClick={() => navigate('/admin/enrich-stores')} className="px-8">
                <Store size={20} /> 매장 정보 보강
              </Button> : <Button variant="gradient" size="lg" onClick={() => navigate('/admin/setup')} className="px-8">
  ...
  return /* pb-20: 모바일 하단 네비게이션 여백 */<div className="space-y-4 pb-20 md:pb-6 px-0">
      ...
      <div className="flex items-center justify-between px-1 pt-1">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {!isMultiView && stores.length > 1 && <select aria-label="매장 선택" value={selectedStore?.id || ''} onChange={e => changeStore(parseInt(e.target.value))} className="text-[10px] font-bold bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-slate-300 outline-none max-w-[100px]">
                {stores.map(s => <option key={s.id} value={s.id} className="bg-slate-900">
                    {s.name}
                  </option>)}
              </select>}
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {/* 매장 추가 */}
          <button type="button" onClick={() => navigate('/admin/setup')} aria-label="매장 추가" className="flex items-center gap-1 h-9 px-3 bg-gradient-to-r from-orange-500 to-rose-600 text-white rounded-xl text-xs font-black shadow-lg shadow-orange-500/20 hover:brightness-105 active:scale-95 transition-all">
            <Plus size={15} aria-hidden="true" />{' '}
            <span className="hidden sm:inline">매장 추가</span>
          </button>
          {/* 대기 주문 알림 */}
          <button type="button" onClick={() => handleNav('orders')} aria-label={pendingCount > 0 ? `대기 주문 ${pendingCount}건 보기` : '주문서로 이동'} className="relative p-2.5 bg-white/5 border border-white/10 rounded-xl">
            <Bell size={16} aria-hidden="true" className={pendingCount > 0 ? 'text-orange-400' : 'text-slate-500'} />
            {pendingCount > 0 && <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-orange-500 text-white text-[9px] font-black rounded-full flex items-center justify-center px-1 tabular-nums">
                {pendingCount}
              </span>}
          </button>
          {/* 새로고침 */}
          <button type="button" onClick={() => isMultiView ? fetchMultiStoreData() : fetchStoreData(selectedStore?.id)} aria-label="새로고침" aria-busy={refreshing} className="p-2.5 bg-white/5 border border-white/10 rounded-xl">
      ...
      <AnimatePresence>
        {anomalyAlert && <motion.div initial={{
        opacity: 0,
        y: -20
      }} animate={{
        opacity: 1,
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-left space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-4 pt-1.5 font-mono text-[10px] text-slate-500 font-bold">
                  <span>
                    Z-Score:{' '}
                    <strong className="text-rose-400">{anomalyAlert.zScore?.toFixed(2)}</strong>
                  </span>
                  <span>
                    현재 1시간 주문:{' '}
                    <strong className="text-rose-400">{anomalyAlert.currentHourOrders}건</strong>
                  </span>
                  <span>
                    24시간 평균:{' '}
                    <strong className="text-slate-400">{anomalyAlert.mean?.toFixed(1)}건</strong>
                  </span>
                </div>
              </div>
            </div>
            <button onClick={() => setAnomalyAlert(null)} className="p-1.5 hover:bg-rose-500/10 rounded-lg text-slate-400 hover:text-rose-200 transition-colors shrink-0 relative z-10">
              <X size={16} />
            </button>
          </motion.div>}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {activeCalls.length > 0 && !isMultiView && <motion.div initial={{
        opacity: 0,
        y: -10
      }} animate={{
        opacity: 1,
        ...
        y: -10
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-black text-orange-400">
                  실시간 매장 직원 호출 현황 ({activeCalls.length}건)
                </h3>
              </div>
            </div>
            ...
            return <div key={call.id} className="p-3 bg-slate-950/60 border border-white/5 rounded-xl flex items-center justify-between text-xs gap-3">
                    <div className="min-w-0 flex-1 text-left">
                      <p className="font-bold text-slate-200">
                        🛎️ {parsedData.tableName || '포장'}번 테이블 호출 :{' '}
                        <strong className="text-orange-400">
                          "{parsedData.type || '직원 호출'}"
                        </strong>
                      </p>
                      <div className="flex items-center gap-3 mt-1 text-[10px] font-mono text-slate-500 font-bold">
                        <span>
                          {new Date(call.created_at).toLocaleTimeString('ko-KR', {
                      hour12: false
                    })}
                        </span>
                        <span>·</span>
                    ...
                    <button onClick={() => markAsRead(call.id)} className="px-3 h-8 rounded-lg bg-orange-500 hover:bg-orange-600 text-slate-950 font-black text-[10px] tracking-wider transition-all active:scale-95 shrink-0 flex items-center gap-1 shadow-md shadow-orange-500/10">
                      <span>호출 확인</span>
                    </button>
                  </div>;
          })}
      ...
      <div className="flex items-center gap-2 px-1">
        ...
        <div className="flex bg-white/5 border border-white/10 rounded-xl p-1 flex-shrink-0">
          <button onClick={() => setIsMultiView(false)} className={`px-2.5 py-1.5 text-[10px] font-black rounded-lg transition-all ${!isMultiView ? 'bg-orange-500 text-white' : 'text-slate-500'}`}>
            단일
          </button>
          {stores.length > 1 && <button onClick={() => setIsMultiView(true)} className={`px-2.5 py-1.5 text-[10px] font-black rounded-lg transition-all ${isMultiView ? 'bg-indigo-500 text-white' : 'text-slate-500'}`}>
              전체
            </button>}
        </div>
        {/* 기간 탭 */}
        <div className="flex flex-1 bg-white/5 border border-white/10 rounded-xl p-1">
          {[['today', '오늘'], ['week', '주간'], ['month', '월간']].map(([v, l]) => <button key={v} onClick={() => setTimeRange(v)} className={`flex-1 py-1.5 text-[10px] font-black rounded-lg transition-all ${timeRange === v ? 'bg-white/10 text-white' : 'text-slate-500'}`}>
              {l}
            </button>)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 xs:grid-cols-2 gap-2.5 sm:gap-3 px-1">
        ...
        value: stats?.total_orders > 0 ? `${Math.round((stats.by_status?.completed || 0) / stats.total_orders * 100)}%` : '0%',
        trend: undefined,
        ...
        trend: undefined,
        accent: pendingCount > 0 ? 'border-orange-500/40' : 'border-violet-500/20'
      }].map(({
        title,
        icon: Icon,
        color,
        ...
        accent
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-1">
                <p className="text-[9px] sm:text-[11px] text-slate-500 font-bold truncate">
                  {title}
                </p>
                {trend !== undefined && <div className={`shrink-0 flex items-center text-[8px] sm:text-[9px] font-black px-1 py-0.5 rounded-md gap-0.5 ${trend >= 0 ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'}`}>
                    {trend >= 0 ? <ArrowUpRight size={8} /> : <ArrowDownRight size={8} />}
                    {Math.abs(trend)}%
                  </div>}
              </div>
              <p className="text-base sm:text-lg font-black text-white leading-tight tabular-nums truncate">
                {value}
              </p>
            </div>
          </div>)}
      ...
      {selectedStore && !isMultiView && <div className="space-y-4 px-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Suspense fallback={<SkeletonChart />}>
              <SalesForecastWidget storeId={selectedStore.id} refreshKey={forecastRefreshKey} />
            </Suspense>
          </div>
      ...
      <div className="px-1">
        ...
        <div className="grid grid-cols-3 xs:grid-cols-4 md:grid-cols-6 gap-2 sm:gap-2.5">
          ...
          color,
          badge
        }) => <button key={path} onClick={() => handleNav(path)} className="relative flex flex-col items-center gap-1.5 p-2.5 sm:p-3 bg-white/5 border border-white/10 rounded-2xl hover:border-white/20 hover:bg-white/10 transition-all active:scale-95">
              ...
              <span className="text-[9px] sm:text-[11px] font-black text-slate-400 text-center leading-tight">
                {label}
              </span>
              {badge > 0 && <span className="absolute -top-1 -right-1 min-w-[18px] h-4.5 bg-orange-500 text-white text-[8px] sm:text-[10px] font-black rounded-full flex items-center justify-center px-1 shadow-lg border border-slate-900">
                  {badge}
                </span>}
            </button>)}
        </div>
        ...
        <div className="flex items-center justify-between mb-3 px-1">
          <h2 className="text-sm font-black text-white flex items-center gap-2">
            {isMultiView ? <>
                ...
                실시간 주문
                {pendingCount > 0 && <span className="ml-1 px-1.5 py-0.5 bg-orange-500 text-white text-[9px] font-black rounded-md">
                    대기 {pendingCount}
                  </span>}
              </>}
          </h2>
          {!isMultiView && <Link to={`/admin/stores/${selectedStore?.id}/orders`} className="text-[10px] font-black text-orange-400 flex items-center gap-1 hover:text-orange-300 transition-colors">
              전체 <ArrowUpRight size={12} />
            </Link>}
        </div>

        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
          {isMultiView ? multiViewLoading ? <div className="flex flex-col items-center justify-center py-16 gap-3">
                <div className="w-7 h-7 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
                <p className="text-[11px] text-slate-500 font-bold">전체 매장 집계 중...</p>
              </div> : multiViewError ? <div className="flex flex-col items-center justify-center py-16 gap-3">
                <AlertCircle size={32} className="text-rose-400/50" />
                <p className="text-xs text-slate-500">데이터를 불러오지 못했습니다</p>
                <button onClick={fetchMultiStoreData} className="px-4 py-2 text-xs font-black text-indigo-400 border border-indigo-500/30 rounded-xl hover:bg-indigo-500/10">
                  다시 시도
                </button>
              </div> : multiStoreStats?.stores?.length > 0 ? multiStoreStats.stores.map((s, idx) => <div key={s.store_id} className="px-4 py-3.5 flex items-center justify-between hover:bg-white/5 transition-all border-b border-white/5 last:border-0">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 bg-white/10 text-white rounded-xl flex items-center justify-center font-black text-xs flex-shrink-0">
                      {idx + 1}
                    </div>
                    <div className="min-w-0">
                ...
                width: `${s.total_sales / (multiStoreStats.summary?.total_sales || 1) * 100}%`
              }} />
                    </div>
                  </div>
              ...
              </div> : recentOrders.length === 0 ? <div className="py-16 text-center px-6 space-y-3">
              ...
              <p className="text-slate-500 text-xs">
                QR 스캔 후 고객이 주문하면 여기에 표시됩니다.
              </p>
            </div> : <div className="divide-y divide-white/5">
              {recentOrders.map(order => <Link key={order.id} to={`/admin/stores/${selectedStore?.id}/orders`} className="px-4 py-4 flex items-center gap-3 hover:bg-white/5 transition-all active:bg-white/10 block">
                  <div className="relative flex-shrink-0">
                    ...
                    {order.status === 'pending' && <span className="absolute -top-0.5 -right-0.5 flex h-2.5 w-2.5">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75" />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[9px] bg-white/10 text-slate-400 px-1 py-0.5 rounded font-bold flex-shrink-0">
      ...
      {/* ── 데이터 내보내기 ── */}
      {selectedStore && !isMultiView && <ExportPanel storeId={selectedStore.id} />}
      ...
      <div className="hidden md:block px-1">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          navTo: '/admin/tinkerbell'
        }].map(tool => <button key={tool.label} onClick={() => tool.navTo ? navigate(tool.navTo) : handleNav(tool.path)} className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 hover:border-white/20 transition-all group text-left">
              ...
              <div className="min-w-0">
                <p className="font-black text-white text-sm group-hover:text-orange-400 transition-colors">
                  {tool.label}
                </p>
      ...
      {/* ── AI 팅커벨 배너 (데스크톱) ── */}
      <div className="hidden md:block rounded-3xl p-7 text-white relative overflow-hidden border border-white/10 mx-1" style={{
      background: 'linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#0f172a 100%)'
    }}>
        <div className="absolute -top-10 -right-10 w-44 h-44 rounded-full opacity-20 blur-2xl" style={{
        ...
        <div className="relative z-10 flex items-center justify-between gap-6">
          <div>
            ...
            <h4 className="text-xl font-black mb-2">
              AI 매출 예측 · <span className="text-indigo-400">팅커벨</span>
            </h4>
            <p className="text-[11px] text-slate-400 leading-relaxed max-w-xs">
              고객 방문 패턴 분석, 피크타임 예측, 인기 메뉴 자동 추천으로 매출을 극대화하세요.
            </p>
          </div>
          <button onClick={() => navigate('/admin/tinkerbell')} className="flex-shrink-0 px-6 py-3 bg-indigo-500 hover:bg-indigo-400 text-white rounded-2xl font-black text-sm transition-all whitespace-nowrap">
            지금 시작
          </button>
        </div>
      </div>
