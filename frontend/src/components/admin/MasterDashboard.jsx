...
import {
    ...
    BarChart, Bar, Cell, PieChart as RechartsPieChart, Pie, Line, ComposedChart
} from 'recharts';
...
const SalesTrendChart = ({ storeId }) => {
    ...
    useEffect(() => {
        ...
        analyticsAPI.getSales(storeId, 'daily', start, end)
            .then(res => {
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-3">
                <ResponsiveContainer width="100%" height={170}>
                    <AreaChart data={data}>
                        ...
                        <XAxis dataKey="date" tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} tickFormatter={v => v?.slice(5) ?? v} />
                        <YAxis tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} width={45} tickFormatter={v => formatCompactPrice(v)} />
                        <Tooltip
                            contentStyle={{ backgroundColor: '#0F172A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, fontSize: 11, color: '#e2e8f0' }}
                            formatter={(v) => [formatPrice(v), '매출']}
                        />
                        <Area type="monotone" dataKey="sales" stroke="#f97316" fill="url(#salesGrad)" strokeWidth={2} dot={false} activeDot={{ r: 3, fill: '#f97316' }} />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
        </div>
...
/* ─── 주문 상태 분포 ─── */
const OrderStatusDonut = ({ stats }) => {
    const byStatus = stats?.by_status;
    if (!byStatus) return null;

    const colorMap = { completed: '#10b981', pending: '#f97316', preparing: '#f59e0b', ready: '#34d399', cancelled: '#f43f5e', confirmed: '#3b82f6', paid: '#14b8a6' };
    const labelMap = { completed: '완료', pending: '대기', preparing: '조리중', ready: '준비완료', cancelled: '취소', confirmed: '확인', paid: '신규' };
                ...
                <div className="flex flex-wrap justify-center gap-x-3 gap-y-1 mt-1">
                    {data.map(d => (
                        <div key={d.name} className="flex items-center gap-1">
                            ...
                            <span className="text-[9px] text-slate-400 font-bold">{d.name} <span className="text-white">{d.value}</span></span>
                        </div>
                    ))}
                </div>
            </div>
...
const PeakHoursBar = ({ storeId }) => {
    ...
    useEffect(() => {
        ...
        ordersAPI.getDetailedStats(storeId, start, end)
            .then(res => {
        ...
        <div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-3">
                <ResponsiveContainer width="100%" height={140}>
                    <BarChart data={data}>
                        <CartesianGrid stroke="rgba(255,255,255,0.03)" vertical={false} />
                        <XAxis dataKey="hour" tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} tickFormatter={v => `${v}시`} />
                        <YAxis tick={{ fill: '#64748b', fontSize: 9 }} axisLine={false} tickLine={false} width={25} />
                        <Tooltip
                            contentStyle={{ backgroundColor: '#0F172A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, fontSize: 11, color: '#e2e8f0' }}
                            formatter={(v) => [v + '건', '주문']}
                        />
                        <Bar dataKey="count" radius={[4, 4, 0, 0]} maxBarSize={20}>
                            {data.map((entry, idx) => (
                                <Cell key={idx} fill={(entry.count || 0) > maxCount * 0.7 ? '#a78bfa' : '#6366f1'} />
                            ))}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
...
/* ─── AI 매출 예측 ─── */
const SalesForecastWidget = ({ storeId }) => {
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        if (!storeId) return;
        setLoading(true);
        analyticsAPI.getForecast(storeId, 7)
            .then(res => {
                ...
                <ResponsiveContainer width="100%" height={140}>
                    <ComposedChart data={data}>
                        ...
                        <Tooltip
                            contentStyle={{ backgroundColor: '#0F172A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, fontSize: 11, color: '#e2e8f0' }}
                            formatter={(v, name) => {
                                const labels = { predicted: '예측 매출', confidence_upper: '상한', confidence_lower: '하한' };
                                return [formatPrice(v), labels[name] || name];
                            }}
                        />
                        <Area type="monotone" dataKey="confidence_upper" stroke="none" fill="url(#forecastGrad)" />
                        <Area type="monotone" dataKey="confidence_lower" stroke="none" fill="#0F172A" />
                        <Line type="monotone" dataKey="predicted" stroke="#818cf8" strokeWidth={2} dot={false} strokeDasharray="4 3" />
                    </ComposedChart>
                </ResponsiveContainer>
                <p className="text-[9px] text-slate-600 text-center mt-1 font-bold">점선: AI 예측 · 음영: 신뢰 구간</p>
            </div>
        </div>
    );
};
