...
const MasterDashboard = () => {
    ...
    const fetchStoreData = useCallback(async (storeId) => {
        try {
            const [statsRes, ordersRes, compRes] = await Promise.allSettled([
            ...
            if (ordersRes.status === 'fulfilled') {
                const o = ordersRes.value?.data ?? ordersRes.value;
                setRecentOrders(Array.isArray(o) ? o.slice(0, 8) : []);
            }
    ...
    const fetchMultiStoreData = useCallback(async () => {
        ...
        try {
            ...
            if (timeRange === 'today') {
                ...
                start = w.toISOString(); end = now.toISOString();
            } else {
                const m = new Date(now); m.setMonth(m.getMonth() - 1);
                start = m.toISOString(); end = now.toISOString();
            }
            ...
            setStats({
                ...
                by_status: { completed: d?.summary?.total_orders ?? 0 },
            });
        } catch (err) {
            console.error('다점포 데이터 로딩 실패:', err);
    ...
    const handleToolNav = useCallback((toolPath) => {
        ...
        if (store?.id) navigate(`/admin/stores/${store.id}/${toolPath}`);
        else navigate('/admin/stores/new');
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[0,1,2,3].map(i => (
                        <div key={i} className="bg-white/5 rounded-2xl p-5 border border-white/10 space-y-3">
                            ...
                            <div className="h-8 w-28 bg-white/15 rounded-xl animate-pulse" />
                            <div className="h-3 w-16 bg-white/10 rounded animate-pulse" />
                        </div>
                    ))}
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-2 bg-white/5 rounded-2xl p-5 border border-white/10 h-64 animate-pulse" />
    ...
    const StatCard = ({ title, value, icon: Icon, trend, color, bgColor, progress }) => (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 md:p-6 relative overflow-hidden group hover:-translate-y-1 transition-transform">
            <div className={`absolute -top-8 -right-8 w-28 h-28 rounded-full blur-2xl opacity-10 group-hover:opacity-20 transition-opacity ${bgColor}`} />
            <div className="flex justify-between items-start mb-3">
                <div className={`p-2.5 rounded-xl bg-white/5 ${color}`}>
                    <Icon size={20} />
                </div>
                {trend !== undefined && (
                    <div className={`flex items-center text-[10px] font-black px-2 py-1 rounded-lg gap-0.5 ${trend >= 0 ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'}`}>
                        {trend >= 0 ? <ArrowUpRight size={11} /> : <ArrowDownRight size={11} />}
                        {Math.abs(trend)}%
                    </div>
                )}
            </div>
            <p className="text-slate-400 text-[10px] md:text-xs font-bold tracking-wider mb-1">{title}</p>
    ...
    return (
        <div className="space-y-6 lg:space-y-10">
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-[10px] font-black rounded-lg border transition-all ${isMultiView ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white/5 text-slate-400 border-white/10 hover:border-white/20'}`}
                        >전체 매장 통계</button>
                    </div>
                </div>
                    ...
                    <div className="flex bg-white/5 p-1 rounded-xl">
                        {['today', 'week', 'month'].map(range => (
                            <button
                                ...
                                className={`px-3 md:px-5 py-2 text-xs font-black rounded-lg transition-all ${timeRange === range ? 'bg-white text-slate-900 shadow' : 'text-slate-400 hover:text-slate-200'}`}
                            >
                    ...
                    {!isMultiView && (
                        <div className="relative flex-1 min-w-[140px]">
                            <select
                                ...
                                className="w-full pl-4 pr-8 py-3 bg-white/5 border border-white/10 rounded-xl text-white font-black text-xs outline-none cursor-pointer appearance-none hover:border-white/20"
                            >
                                {stores.map(store => (
                                    <option key={store.id} value={store.id} className="bg-slate-900">{store.name}</option>
                                ))}
                            </select>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                <StatCard title="총 매출" value={formatPrice(stats?.total_sales || 0)} icon={DollarSign} trend={comparison?.growth?.sales} color="text-orange-400" bgColor="bg-orange-500" progress={100} />
                <StatCard title="총 주문 수" value={`${stats?.total_orders || 0}건`} icon={ShoppingBag} trend={comparison?.growth?.orders} color="text-indigo-400" bgColor="bg-indigo-500" progress={stats?.total_orders > 0 ? 100 : 0} />
                <StatCard
                    title="완료율"
                    value={stats?.total_orders > 0 ? `${Math.round(((stats.by_status?.completed || 0) / stats.total_orders) * 100)}%` : '0%'}
                    icon={Target} color="text-amber-400" bgColor="bg-amber-500"
                    progress={stats?.total_orders > 0 ? (stats.by_status?.completed || 0) / stats.total_orders * 100 : 0}
                />
                <StatCard title="매장 상태" value={stats?.total_orders > 0 ? '활성' : '대기'} icon={Activity} color="text-emerald-400" bgColor="bg-emerald-500" progress={stats?.total_orders > 0 ? 85 : 10} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-xl font-black text-white flex items-center gap-3">
                            {isMultiView ? (
                                <><TrendingUp className="text-indigo-400" size={22} />매장별 실적 순위</>
                            ) : (
                                <>
                        ...
                        {!isMultiView && (
                            <Link to={`/admin/stores/${selectedStore?.id}/orders`} className="text-[10px] font-black text-orange-400 flex items-center gap-1 hover:text-orange-300 transition-colors">
                                전체 내역 <ArrowUpRight size={14} />
                            </Link>
                        )}
                    </div>

                    <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden min-h-[200px] lg:min-h-[500px]">
                        {isMultiView ? (
                            <div>
                                {multiViewLoading ? (
                                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                                        <div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">전체 매장 데이터 집계 중...</p>
                                    </div>
                                ) : multiViewError ? (
                                    <div className="flex flex-col items-center justify-center py-24 gap-3">
                                        <div className="text-3xl">⚠️</div>
                                        <p className="text-xs font-bold text-slate-400">데이터를 불러오지 못했습니다</p>
                                        <button onClick={fetchMultiStoreData} className="mt-2 px-4 py-2 text-xs font-black text-indigo-400 border border-indigo-500/30 rounded-xl hover:bg-indigo-500/10 transition-colors">다시 시도</button>
                                    </div>
                                ) : multiStoreStats?.stores?.length > 0 ? (
                                    multiStoreStats.stores.map((s, idx) => (
                                        <div key={s.store_id} className="p-6 flex items-center justify-between hover:bg-white/5 transition-all border-b border-white/5 last:border-0">
                                            <div className="flex items-center gap-6">
                                                <div className="w-10 h-10 bg-white/10 text-white rounded-xl flex items-center justify-center font-black text-sm">{idx + 1}</div>
                                                <div>
                                                    <div className="font-black text-white text-lg">{s.store_name}</div>
                                                    <div className="text-[10px] text-slate-400 font-bold mt-0.5">총 {s.total_orders}건</div>
                                                </div>
                                            </div>
                            ...
                            <div className="divide-y divide-white/5">
                                {recentOrders.length === 0 ? (
                                    ...
                                    <div key={order.id} className="px-4 md:px-6 py-3 md:py-4 flex items-center justify-between hover:bg-white/5 transition-all group">
                                        <div className="flex items-center gap-3 md:gap-5 min-w-0">
                                            <div className="relative flex-shrink-0">
                                                <div className={`w-10 h-10 md:w-14 md:h-14 rounded-2xl flex items-center justify-center font-black text-[10px] md:text-xs ${order.status === 'pending' ? 'bg-orange-500 text-white' : 'bg-white/10 text-slate-400'}`}>
                                                    #{order.order_number?.slice(-3)}
                                                </div>
                                                {order.status === 'pending' && (
                                                    <span className="absolute -top-1 -right-1 flex h-3.5 w-3.5">
                                                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75" />
                                                        <span className="relative inline-flex rounded-full h-3.5 w-3.5 bg-orange-500 border-2 border-slate-950" />
                                                    </span>
                                                )}
                                            </div>
                                            <div className="min-w-0">
                                                <div className="font-black text-white group-hover:text-orange-400 transition-colors flex items-center gap-2 text-sm md:text-base truncate">
                                                    {order.table_name || '포장'}
                                                    <span className="text-[9px] bg-white/10 text-slate-400 px-1.5 py-0.5 rounded-md font-bold flex-shrink-0">{order.customer_name || '비회원'}</span>
                                                </div>
                                        ...
                                        <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
                                            ...
                                            <Link to={`/admin/stores/${selectedStore?.id}/orders`} className="p-2 md:p-2.5 bg-white/5 text-slate-500 rounded-xl group-hover:bg-orange-500 group-hover:text-white transition-all">
                ...
                <div className="space-y-8">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-xl font-black text-white flex items-center gap-3">
                            <Zap className="text-orange-400" size={22} />운영 도구
                        </h2>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-1 gap-3">
                        {[
                            ...
                            <button
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-2xl p-3 md:p-4 flex items-center gap-3 md:gap-4 hover:bg-white/10 hover:border-white/20 transition-all group text-left"
                            >
                                ...
                                <div className="flex-grow min-w-0">
                                    <div className="font-black text-white text-xs md:text-sm group-hover:text-orange-400 transition-colors truncate">{tool.label}</div>
                                    <div className="text-[10px] text-slate-500 mt-0.5 hidden sm:block">{tool.desc}</div>
                                </div>
                                <ChevronRight size={14} className="text-slate-600 group-hover:text-orange-400 transition-colors flex-shrink-0 hidden md:block" />
                            </button>
                        ))}
                    </div>
                    ...
                    {/* 프리미엄 배너 */}
                    <div
                        className="rounded-3xl p-7 text-white relative overflow-hidden shadow-xl border border-white/10"
                    ...
                    >
                        ...
                        <div className="absolute -bottom-6 -left-6 w-32 h-32 rounded-full opacity-15 blur-2xl" style={{ background: 'radial-gradient(circle, #f97316, transparent)' }} />
                        <div className="relative z-10">
                            ...
                            <h4 className="text-xl font-black mb-2 leading-tight">AI 매출 예측<br /><span className="text-indigo-400">인사이트</span></h4>
                            <p className="text-[11px] text-slate-400 leading-relaxed mb-6">고객 방문 패턴 분석, 인기 메뉴 자동 추천, 피크타임 예측으로 매출을 극대화하세요.</p>
                            <button className="w-full py-3.5 bg-indigo-500 hover:bg-indigo-400 text-white rounded-2xl font-black text-sm transition-all">지금 시작하기</button>
                        </div>
                    </div>
                </div>
            </div>
