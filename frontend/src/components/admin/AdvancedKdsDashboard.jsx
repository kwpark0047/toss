  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 flex flex-col h-screen select-none">
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* 수동 리프레시 버튼 */}
          <button onClick={() => fetchKdsOrders(true)} disabled={loading} className="p-2 rounded-lg border border-slate-800 bg-slate-900 hover:bg-slate-850 text-slate-300 disabled:opacity-50 transition-all">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 my-3">
        ...
        <div className="flex bg-slate-900 p-0.5 rounded-lg border border-slate-800">
          ...
          id: 'TAKEOUT',
          label: '포장 주문'
        }].map(opt => <button key={opt.id} onClick={() => setFilteredType(opt.id)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${filterType === opt.id ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>
              {opt.label}
            </button>)}
        </div>
    ...
    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 overflow-hidden min-h-0">
          ...
          <div className="bg-slate-900/30 rounded-xl border border-slate-900 flex flex-col min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 min-h-0">
              {pendingOrders.length === 0 ? <div className="h-full flex flex-col items-center justify-content justify-center text-center text-slate-600 py-10">
                  <Clock className="size-8 stroke-[1.5] mb-2 opacity-50" />
                  <p className="text-xs">대기 중인 신규 주문이 없습니다.</p>
                </div> : pendingOrders.map(order => <div key={order.id} className="bg-slate-900 border border-amber-500/20 hover:border-amber-500/40 rounded-lg overflow-hidden transition-all shadow-sm">
                    <div className="p-3 border-b border-slate-850/60 bg-amber-500/[0.02] flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-sm text-slate-100">
                          #{order.order_number.slice(-4)}
                        </span>
                        <span className={`text-[10px] px-2 py-0.5 rounded font-medium ${order.is_takeout ? 'bg-orange-500/10 text-orange-400' : 'bg-blue-500/10 text-blue-400'}`}>
                    ...
                    <div className="p-3 flex flex-col gap-2">
                      ...
                      {order.notes && <div className="text-[10px] bg-slate-950 p-2 rounded text-slate-400 border border-slate-900 leading-relaxed">
          ...
          <div className="bg-slate-900/30 rounded-xl border border-slate-900 flex flex-col min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 min-h-0">
              {preparingOrders.length === 0 ? <div className="h-full flex flex-col items-center justify-content justify-center text-center text-slate-600 py-10">
                  <Printer className="size-8 stroke-[1.5] mb-2 opacity-50" />
                  <p className="text-xs">조리 중인 주문이 없습니다.</p>
                </div> : preparingOrders.map(order => <div key={order.id} className="bg-slate-900 border border-sky-500/20 hover:border-sky-500/40 rounded-lg overflow-hidden transition-all shadow-sm">
                  ...
                  return <li key={item.id} onClick={() => toggleCheckItem(order.id, item.id)} className={`py-1.5 flex flex-col cursor-pointer transition-all ${isChecked ? 'opacity-40 line-through' : ''}`}>
                      ...
                      <div className="flex gap-1.5 mt-2">
                        <button onClick={() => handleReprintSlip(order.id)} disabled={updatingId === order.id} title="주방 인쇄 작업 강제 추가" className="px-2.5 bg-slate-950 hover:bg-slate-900 border border-slate-850 hover:border-slate-750 disabled:opacity-50 text-slate-400 hover:text-slate-200 rounded font-medium text-xs flex items-center justify-center transition-all">
          ...
          <div className="bg-slate-900/30 rounded-xl border border-slate-900 flex flex-col min-h-0">
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 min-h-0">
              {readyOrders.length === 0 ? <div className="h-full flex flex-col items-center justify-content justify-center text-center text-slate-600 py-10">
                  <CheckCircle2 className="size-8 stroke-[1.5] mb-2 opacity-50" />
                  <p className="text-xs">호출 대기 중인 완료 메뉴가 없습니다.</p>
                </div> : readyOrders.map(order => <div key={order.id} className="bg-slate-900 border border-emerald-500/20 hover:border-emerald-500/40 rounded-lg overflow-hidden transition-all shadow-sm">
