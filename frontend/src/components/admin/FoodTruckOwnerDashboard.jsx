...
useEffect(() => {
  ...
  }, [storeId, fetchTruckDetails, stopAutoSync]);
...
const json = await api.post(`/foodtruck/stores/${storeId}/session`, {
      ...
      setTruckInfo(prev => ({
        ...prev,
        is_active_session: updated.is_active_session
      }));
    } catch (err) {
      alert(err.message);
    }
...
const json = await api.post(`/foodtruck/stores/${storeId}/emergency-soldout`, {
      ...
      setTruckInfo(prev => ({
        ...prev,
        is_sold_out_emergency: updated.is_sold_out_emergency
      }));
    } catch (err) {
      alert(err.message);
    }
...
const handleSpotSelect = (lat, lng, spotName) => {
  ...
  if (loading) {
    return <div className="flex flex-col items-center justify-center p-12 min-h-[400px] text-slate-400">
  ...
  return <div className="space-y-8 text-slate-100 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl bg-slate-900 border border-slate-850 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex gap-2">
          <span className={`px-3 py-1.5 rounded-xl text-xs font-black border ${isSessionOn ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
            {isSessionOn ? '영업 중' : '영업 마감'}
          </span>
          <span className={`px-3 py-1.5 rounded-xl text-xs font-black border ${isSoldOutOn ? 'bg-rose-500/10 text-rose-400 border-rose-500/20 animate-pulse' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* 영업 세션 제어 카드 */}
        <div className={`p-6 rounded-3xl border transition-all flex flex-col justify-between h-56 ${isSessionOn ? 'bg-slate-900/60 border-emerald-500/30' : 'bg-slate-900/20 border-slate-900'}`}>
          <div>
          ...
          <button onClick={handleToggleSession} className={`w-full py-3.5 rounded-2xl text-xs font-black transition-all active:scale-95 ${isSessionOn ? 'bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700' : 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-xl shadow-emerald-500/10'}`}>
            {isSessionOn ? '영업 일시 중지 / 마감하기' : '지금 즉시 영업 개시 (Open)'}
          </button>
        </div>

        {/* 긴급 품절 킬스위치 카드 */}
        <div className={`p-6 rounded-3xl border transition-all flex flex-col justify-between h-56 ${isSoldOutOn ? 'bg-rose-950/20 border-rose-500/50' : 'bg-slate-900/20 border-slate-900'}`}>
          <div>
            <div className="flex items-center justify-between">
          ...
          <button onClick={handleToggleEmergencySoldOut} className={`w-full py-3.5 rounded-2xl text-xs font-black transition-all active:scale-95 ${isSoldOutOn ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-gradient-to-r from-orange-500 to-rose-600 hover:from-orange-600 hover:to-rose-700 text-white shadow-xl shadow-orange-500/10'}`}>
      ...
      <div className="p-6 rounded-3xl bg-slate-900 border border-slate-850 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button onClick={toggleAutoSync} className={`flex items-center justify-center gap-2 px-6 h-12 rounded-xl text-xs font-bold transition-all active:scale-95 border ${autoSync ? 'bg-orange-500/15 text-orange-500 border-orange-500/30' : 'bg-slate-950/40 text-slate-400 border-slate-800'}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2.5 pt-1">
            <button onClick={() => handleSpotSelect(37.5562, 126.9224, '홍대 걷고싶은거리')} className="px-5 h-11 rounded-xl bg-slate-950/30 border border-slate-800 text-xs font-bold hover:border-orange-500/40 text-slate-300 hover:text-white transition-all active:scale-95 flex items-center justify-center">
              🚩 홍대입구
            </button>
            <button onClick={() => handleSpotSelect(37.4979, 127.0276, '강남역 11번 출구 스퀘어')} className="px-5 h-11 rounded-xl bg-slate-950/30 border border-slate-800 text-xs font-bold hover:border-orange-500/40 text-slate-300 hover:text-white transition-all active:scale-95 flex items-center justify-center">
              🚩 강남역 사거리
            </button>
            <button onClick={() => handleSpotSelect(37.5822, 127.0018, '대학로 마로니에 예술공원')} className="px-5 h-11 rounded-xl bg-slate-950/30 border border-slate-800 text-xs font-bold hover:border-orange-500/40 text-slate-300 hover:text-white transition-all active:scale-95 flex items-center justify-center">
              🚩 대학로
            </button>
            <button onClick={() => handleSpotSelect(35.1585, 129.0620, '부산 서면 쥬디스태화 스퀘어')} className="px-5 h-11 rounded-xl bg-slate-950/30 border border-slate-800 text-xs font-bold hover:border-orange-500/40 text-slate-300 hover:text-white transition-all active:scale-95 flex items-center justify-center">
              🚩 부산 서면
            </button>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-slate-850/60 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="text" placeholder="예: 여의도 한강시민공원 멀티플라자 광장 앞" value={manualAddress} onChange={e => setManualAddress(e.target.value)} className="flex-1 px-4 py-3 rounded-xl bg-slate-950/40 border border-slate-800 text-white placeholder-slate-500 text-xs font-bold focus:outline-none focus:border-orange-500/40 focus:ring-4 focus:ring-orange-500/5 transition-all" />
            <button onClick={() => handleUpdateGpsManually(manualCoords.lat, manualCoords.lng)} disabled={syncingGps} className="px-6 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-black flex items-center justify-center gap-1.5 transition-all active:scale-95">
              {syncingGps ? <>
