...
const STATUS_META = {
    ...
    low:       { label: '저재고', color: 'text-amber-400',   bg: 'bg-amber-500/10',   icon: AlertCircle,  border: 'border-amber-500/30'  },
    out:       { label: '품절',   color: 'text-red-400',     bg: 'bg-red-500/10',     icon: XCircle,      border: 'border-red-500/30'    },
    unlimited: { label: '무제한', color: 'text-slate-400',   bg: 'bg-slate-500/10',   icon: Package,      border: 'border-slate-500/20'  },
};
...
function StockBadge({ status }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black ${meta.color} ${meta.bg} border ${meta.border}`}>
            <Icon size={10} />
            {meta.label}
        </span>
    );
            ...
            >
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">
                                현재 재고: <span className="text-white font-bold">
                ...
                <div className="p-6">
                    ...
                    <div className="flex gap-2 mb-6 bg-white/5 p-1 rounded-xl">
                        {[{ key: 'adjust', label: '입출고' }, { key: 'set', label: '직접 설정' }].map(t => (
                            <button
                                key={t.key}
                    ...
                    {mode === 'adjust' ? (
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <button onClick={() => setChange(v => String((Number(v) || 0) - 1))}
                            ...
                            <div>
                                <label className="text-[11px] font-black text-slate-400 mb-2 block">메모 (선택)</label>
                                <input
                                    type="text"
                                    value={note}
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="text-[11px] font-black text-slate-400 mb-2 block">저재고 경고 임계치</label>
                                <input
                                    type="number"
                                    value={threshold}
...
function HistoryModal({ product, onClose }) {
    ...
    useEffect(() => {
        inventoryAPI.getProductHistory(product.id).then(res => {
                                ...
                                <div key={h.id} className="flex items-center gap-4 px-6 py-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-xs font-black ${h.change > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                                {h.change > 0 ? `+${h.change}` : h.change}개
                                            </span>
                                        </div>
                                        {h.note && <p className="text-[11px] text-slate-500 mt-0.5 truncate">{h.note}</p>}
                                        <p className="text-[10px] text-slate-600 mt-0.5">
                                            잔여 {h.qty_after}개 · {new Date(h.created_at).toLocaleString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                        </p>
                                    </div>
                                </div>
                            ))}
...
export default function InventoryManager() {
    ...
    const [summary, setSummary] = useState({ total_managed: 0, out_of_stock: 0, low_stock: 0, ok: 0 });
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => { loadInventory(1); }, [loadInventory]);

    const STATUS_FILTERS = [
        { key: 'ALL',       label: '전체',   count: null },
        { key: 'out',       label: '품절',   count: summary.out_of_stock },
        { key: 'low',       label: '저재고', count: summary.low_stock },
        { key: 'ok',        label: '정상',   count: summary.ok },
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => loadInventory(1)} className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-400 text-xs font-black hover:text-white hover:bg-white/10 transition-all">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <SummaryCard icon={Package}       label="재고 관리 메뉴"  value={summary.total_managed}  color="text-slate-300"   bg="bg-slate-800/50" />
                <SummaryCard icon={XCircle}       label="품절"           value={summary.out_of_stock}   color="text-red-400"     bg="bg-red-500/5"    />
                <SummaryCard icon={AlertCircle}   label="저재고 경고"    value={summary.low_stock}      color="text-amber-400"   bg="bg-amber-500/5"  />
                <SummaryCard icon={CheckCircle}   label="재고 정상"      value={summary.ok}             color="text-emerald-400" bg="bg-emerald-500/5"/>
            </div>
            ...
            {(summary.out_of_stock > 0 || summary.low_stock > 0) && (
                <div className="flex items-center gap-3 px-5 py-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl">
                    ...
                    <p className="text-sm text-amber-300 font-bold">
                        {summary.out_of_stock > 0 && <span className="text-red-400">{summary.out_of_stock}개 품절</span>}
                        {summary.out_of_stock > 0 && summary.low_stock > 0 && ' · '}
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-white/5">
                                <th className="text-left px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">메뉴</th>
                                <th className="text-left px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">카테고리</th>
                                <th className="text-center px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">재고 수량</th>
                                <th className="text-center px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">임계치</th>
                                <th className="text-center px-4 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">상태</th>
                                <th className="text-right px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">관리</th>
                            </tr>
                        </thead>
                                    ...
                                    >
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-500">{product.price?.toLocaleString()}원</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4">
                                            <span className="text-xs text-slate-400">{product.categories?.name || '미분류'}</span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <span className={`text-xl font-black ${
                                                status === 'out' ? 'text-red-400' :
                                                status === 'low' ? 'text-amber-400' :
                                                status === 'unlimited' ? 'text-slate-500' :
                                                'text-white'
                                            }`}>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <span className="text-xs text-slate-500">
                                                {product.stock_quantity === null ? '—' : `${product.low_stock_threshold}개`}
                                            </span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
            ...
            <AnimatePresence>
                {adjustTarget && (
                    <AdjustModal
                        ...
                        onSuccess={() => { setAdjustTarget(null); loadInventory(pagination.page); }}
                    />
                )}
                {historyTarget && (
                    <HistoryModal
