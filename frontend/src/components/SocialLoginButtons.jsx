...
const SocialLoginButtons = ({
  ...
  return <div className="mt-8">
      ...
      <div className="space-y-3">
        {error && <div className="p-3 bg-red-50 border border-red-100 text-red-600 rounded-2xl text-xs text-center font-medium">
            {error}
          </div>}

        {KAKAO_KEY && <button type="button" onClick={handleKakao} disabled={loading} className="w-full py-3.5 px-4 bg-[#FEE500] hover:bg-[#FDD800] text-[#191919] rounded-2xl font-bold text-base transition-all disabled:opacity-50 flex items-center justify-center gap-3">
        ...
        {NAVER_KEY && <button type="button" onClick={handleNaver} disabled={loading} className="w-full py-3.5 px-4 bg-[#03C75A] hover:bg-[#02b350] text-white rounded-2xl font-bold text-base transition-all disabled:opacity-50 flex items-center justify-center gap-3">
            <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
        ...
        {GOOGLE_KEY && <button type="button" onClick={handleGoogle} disabled={loading} className="w-full py-3.5 px-4 bg-white hover:bg-slate-50 text-slate-700 rounded-2xl font-bold text-base transition-all disabled:opacity-50 border border-slate-200 shadow-sm flex items-center justify-center gap-3">
