        ...
        try {
            ...
            const updatedOrder = await prisma_1.default.$transaction(async (tx) => {
                ...
                if (status === 'preparing') {
                    ...
                    logger_1.default.info(`[KDS Service] KDS 주방 인쇄 작업 등록 성공: Order #${order.order_number}`);
                }
                return res;
            });
            ...
            if (status === 'ready' && order.customer_phone) {
                setImmediate(async () => {
                    try {
                        ...
                        if (decryptedPhone) {
                            await AlimtalkService_1.default.sendFoodReady(decryptedPhone, order.stores?.name || 'WeMarket', order.order_number, order.tables?.table_number || '픽업대');
                        }
                    }
                    catch (err) {
            ...
            if (io) {
                ...
                logger_1.default.info(`[KDS Socket] KDS 실시간 브로드캐스트 전송 완료: Store ${numericStoreId}, Order ${order.order_number} → ${status}`);
            }
            return {
                ...updatedOrder,
                table_name: order.tables?.table_number || null,
