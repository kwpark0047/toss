...
jest.mock('../../../services/printService', () => ({ createKitchenJob: jest.fn().mockResolvedValue({}) }));
...
describe('OrderService — Haversine 거리 계산', () => {
    test('같은 위치는 0km', () => {
        expect(getDistanceFromLatLonInKm(37.5665, 126.9780, 37.5665, 126.9780)).toBe(0);
    });

    test('약 100m 거리', () => {
        const dist = getDistanceFromLatLonInKm(37.5665, 126.9780, 37.5674, 126.9780);
        expect(dist).toBeGreaterThan(0.05);
        expect(dist).toBeLessThan(0.2);
    });

    test('약 1km 거리', () => {
        const dist = getDistanceFromLatLonInKm(37.5665, 126.9780, 37.5755, 126.9780);
    ...
    test('약 3km 거리 (서울시청 → 강남역)', () => {
        ...
        const dist = getDistanceFromLatLonInKm(37.5665, 126.9780, 37.4980, 127.0276);
...
describe('OrderService — 지오펜싱 통합 테스트', () => {
    ...
    test('매장 위치 정보 없으면 지오펜싱 스킵', async () => {
        Store.findById.mockResolvedValue({ id: 1, name: '테스트매장', latitude: null, longitude: null });
        ...
        const result = await svc.createOrder({
            store_id: '1', total_amount: 5000
        });
        expect(result.id).toBe(1);
    });

    test('500m 이내 주문 허용', async () => {
        Store.findById.mockResolvedValue({ id: 1, latitude: 37.5665, longitude: 126.9780 });
        const Order = require('../../../repositories/Order');
        Order.create.mockResolvedValue({ id: 2, order_number: 'ORD-002' });

        // 약 100m 떨어진 위치
        const result = await svc.createOrder({
            store_id: '1', table_id: '1', total_amount: 5000,
