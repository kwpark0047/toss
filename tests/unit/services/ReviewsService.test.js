...
describe('ReviewsService', () => {
  ...
  describe('getStoreReviews', () => {
    test('매장별 리뷰 목록을 조회한다', async () => {
      prisma.reviews.findMany.mockResolvedValue([{ id: 1, rating: 5, _count: { likes: 3 } }]);
  ...
  describe('createReview', () => {
    ...
    test('이미 리뷰가 있는 주문이면 400 에러를 반환한다', async () => {
      ...
      await expect(svc.createReview({ store_id: 1, order_id: 100 })).rejects.toThrow(
        '이미 리뷰가 등록된 주문입니다.'
      );
    });
  });
  ...
  describe('loadReviewWithPermission', () => {
    ...
    test('리뷰가 없으면 404 에러', async () => {
      prisma.reviews.findUnique.mockResolvedValue(null);
      await expect(
        svc.loadReviewWithPermission(999, { id: 1, role: 'super_admin' })
    ...
    test('매장 권한이 없으면 403 에러', async () => {
      ...
      await expect(svc.loadReviewWithPermission(1, { id: 1, role: 'owner' })).rejects.toThrow(
        '해당 매장에 대한 권한이 없습니다.'
      );
    });
  });
  ...
  describe('setReply', () => {
    test('리플라이를 등록/수정한다', async () => {
      ...
      await svc.setReply(1, '감사합니다', { id: 1, role: 'super_admin' });
