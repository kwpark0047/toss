...
describe('OrderService', () => {
    ...
    describe('createOrder — 지오펜싱', () => {
        beforeEach(() => {
            Store.findById.mockResolvedValue({
                id: 1, name: '테스트매장', latitude: 37.5665, longitude: 126.9780
            });
        });
        ...
        test('매장 반경 500m 이내면 주문 성공', async () => {
            ...
            Product.findById.mockResolvedValue({ id: 100, name: '아메리카노', is_sold_out: false, stock_quantity: 100 });
            Order.create.mockResolvedValue({ id: 1, order_number: 'ORD-001', store_id: 1, table_id: 10, total_amount: 4500, status: 'pending', order_items: [] });

            const result = await svc.createOrder({
                store_id: '1', table_id: '1', table_number: '1',
                items: [{ product_id: 100, quantity: 1 }],
                total_amount: 4500, phone: '01012345678',
        ...
        test('매장 반경 500m 밖이면 403 에러', async () => {
            await expect(svc.createOrder({
                ...
                total_amount: 4500, latitude: 37.7, longitude: 127.0
            })).rejects.toThrow('매장 반경 500m 밖에서는 주문할 수 없습니다.');
        });

        test('테이블 ID 없으면 지오펜싱 검증 스킵', async () => {
            Order.create.mockResolvedValue({ id: 2, order_number: 'ORD-002', store_id: 1, table_id: null, total_amount: 3000, status: 'pending', order_items: [] });
            const result = await svc.createOrder({
                store_id: '1', total_amount: 3000
            });
            expect(result.id).toBe(2);
        });

        test('위도/경도 없으면 지오펜싱 검증 스킵', async () => {
            Order.create.mockResolvedValue({ id: 3, order_number: 'ORD-003', store_id: 1, table_id: 10, total_amount: 3000, status: 'pending', order_items: [] });
            Table.findByStoreAndTable.mockResolvedValue({ id: 10, table_number: '1' });
            const result = await svc.createOrder({
                store_id: '1', table_id: '10', total_amount: 3000
            });
    ...
    describe('createOrder — 쿠폰 검증', () => {
        beforeEach(() => {
            ...
            Product.findById.mockResolvedValue({ id: 100, name: '라떼', is_sold_out: false, stock_quantity: 50 });
            Order.create.mockResolvedValue({ id: 4, order_number: 'ORD-004', store_id: 1, total_amount: 10000, discount_amount: 2000, status: 'pending', order_items: [] });
        });
        ...
        test('사용 완료된 쿠폰이면 400 에러', async () => {
            ...
            await expect(svc.createOrder({
                store_id: '1', user_coupon_id: 3, total_amount: 10000
        ...
        test('최소 주문 미달 시 400 에러', async () => {
            ...
            await expect(svc.createOrder({
                store_id: '1', user_coupon_id: 4, total_amount: 10000
    ...
    describe('createOrder — 재고 검증', () => {
        ...
        test('품절 상품이 있으면 409 에러', async () => {
            Product.findById.mockResolvedValue({ id: 200, name: '만두', is_sold_out: true, stock_quantity: 0 });
            await expect(svc.createOrder({
                store_id: '1', items: [{ product_id: 200, quantity: 1 }], total_amount: 5000
            })).rejects.toThrow('품절');
        });

        test('재고 부족 시 409 에러', async () => {
            Product.findById.mockResolvedValue({ id: 201, name: '떡볶이', is_sold_out: false, stock_quantity: 2 });
            await expect(svc.createOrder({
                store_id: '1', items: [{ product_id: 201, quantity: 5 }], total_amount: 8000
    ...
    describe('createOrder — 매장 미존재', () => {
        test('매장 없으면 404 에러', async () => {
            ...
            await expect(svc.createOrder({
                store_id: '999', total_amount: 5000
    ...
    describe('updateStatus', () => {
        beforeEach(() => {
            Order.findById.mockResolvedValue({ id: 1, status: 'pending', store_id: 1, table_id: 10, customer_phone: 'enc_01012345678', order_number: 'ORD-001' });
            Order.updateStatus.mockResolvedValue({ id: 1, status: 'confirmed', store_id: 1, table_id: 10, customer_phone: 'enc_01012345678', order_number: 'ORD-001' });
            prisma.order_items.findMany.mockResolvedValue([{ product_id: 100, quantity: 2 }]);
            prisma.products.findUnique.mockResolvedValue({ id: 100, name: '아메리카노', store_id: 1, stock_quantity: 50, low_stock_threshold: 5 });
            prisma.tables.findUnique.mockResolvedValue({ table_number: '1' });
            prisma.$transaction.mockImplementation(async (fn) => fn({
                products: {
                    findUnique: jest.fn().mockResolvedValue({ id: 100, name: '아메리카노', store_id: 1, stock_quantity: 50, low_stock_threshold: 5 }),
        ...
        test('confirmed → completed 시 테이블 상태 변경', async () => {
            Order.findById.mockResolvedValue({ id: 1, status: 'confirmed', store_id: 1, table_id: 10, customer_phone: null, order_number: 'ORD-001' });
            Order.updateStatus.mockResolvedValue({ id: 1, status: 'completed', store_id: 1, table_id: 10, customer_phone: null, order_number: 'ORD-001' });
    ...
    describe('_processLoyaltyPoints', () => {
        ...
        test('포인트 설정 비활성화 시 스킵', async () => {
            ...
            await svc._processLoyaltyPoints({ customer_phone: 'enc_01012345678', store_id: 1, total_amount: 10000, id: 1, order_number: 'ORD-001' });
            expect(tx.user_points.findFirst).not.toHaveBeenCalled();
        });

        test('최소 주문 미달 시 스킵', async () => {
            prisma.store_point_settings.findUnique.mockResolvedValue({ is_enabled: true, min_earn_amount: 10000, earn_rate: 5 });
            const tx = mockTx();
            prisma.$transaction.mockImplementation(async (fn) => fn(tx));
            await svc._processLoyaltyPoints({ customer_phone: 'enc_01012345678', store_id: 1, total_amount: 5000, id: 1, order_number: 'ORD-002' });
            expect(tx.user_points.findFirst).not.toHaveBeenCalled();
        });
    });
});
