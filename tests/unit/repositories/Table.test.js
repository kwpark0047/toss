...
describe('Table.updateLayout', () => {
    ...
    test('요청 매장에 속하지 않은 테이블이 있으면 갱신 전에 거부한다', async () => {
        ...
        await expect(Table.updateLayout(42, [
            { id: 10, x: 1, y: 2 },
            { id: 11, x: 3, y: 4 }
    ...
    test('검증된 매장의 테이블만 트랜잭션에서 갱신한다', async () => {
        mockTransactionTables.findMany
            ...
            .mockResolvedValueOnce([{ id: 10, store_id: 42 }, { id: 11, store_id: 42 }]);
        ...
        expect(mockTransactionTables.update).toHaveBeenCalledWith(expect.objectContaining({
            where: { id: 10 },
        ...
        expect(result).toEqual([{ id: 10, store_id: 42 }, { id: 11, store_id: 42 }]);
    });
});
...
describe('Table.occupyActiveForStore', () => {
    test('매장 소유 활성 테이블만 조건부로 점유한다', async () => {
        ...
        await expect(Table.occupyActiveForStore(10, 42, { tables: mockTransactionTables }))
            .resolves.toBe(true);
