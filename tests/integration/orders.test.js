...
describe('Orders Integration Tests', () => {
  ...
  describe('GET /orders/:id - 주문 단일 상세 조회', () => {
    it('should return order details when found', async () => {
      ...
      const res = await request(app).get(`${baseUrl}/101`).expect(200);
    ...
    it('should return 404 when order not found', async () => {
      ...
      const res = await request(app).get(`${baseUrl}/999`).expect(404);
  ...
  describe('GET /orders/customer/history - 고객 주문 내역 조회', () => {
    it('should return 400 when phone and toss_user_key are both missing', async () => {
      const res = await request(app).get(`${baseUrl}/customer/history`).expect(400);
  ...
  describe('GET /orders/store/:storeId - 매장별 주문 목록 조회 (인증 필요)', () => {
    ...
    it('should return empty array when no orders', async () => {
      ...
      const res = await request(app).get(`${baseUrl}/store/999`).expect(200);
  ...
  describe('GET /orders/store/:storeId/stats - 매장 통계 조회 (인증 필요)', () => {
    it('should return stats for store', async () => {
      ...
      const res = await request(app).get(`${baseUrl}/store/1/stats`).expect(200);
  ...
  describe('GET /orders/store/:storeId/detailed-stats - 상세 통계 조회 (인증 필요)', () => {
    it('should return 400 when start_date or end_date is missing', async () => {
      const res = await request(app).get(`${baseUrl}/store/1/detailed-stats`).expect(400);
    ...
    it('should return detailed stats when dates provided', async () => {
      ...
      expect(mockOrderRepository.getDetailedStats).toHaveBeenCalledWith(
        '1',
        '2026-07-01',
        '2026-07-20'
      );
  ...
  describe('PUT /orders/:id/status - 주문 상태 업데이트 (인증 필요)', () => {
    ...
    it('should update order status with default handling when status is missing', async () => {
      ...
      const res = await request(app).put(`${baseUrl}/101/status`).send({ staff_id: 5 }).expect(200);
  ...
  describe('POST /orders/:id/cancel - 주문 취소 (인증 필요)', () => {
    it('should cancel order successfully', async () => {
      ...
      const res = await request(app).post(`${baseUrl}/101/cancel`).expect(200);
  ...
  describe('DELETE /orders/:id - 주문 삭제 (인증 필요)', () => {
    it('should delete order successfully', async () => {
      ...
      const res = await request(app).delete(`${baseUrl}/101`).expect(200);
  ...
  describe('POST /orders - 주문 생성 (검증 미들웨어 필요)', () => {
    it('should create order successfully', async () => {
      const mockOrder = {
        ...
        status: 'pending',
        total_amount: 25000,
        items: [{ product_id: 10, quantity: 2, product: { name: '아메리카노', price: 4500 } }],
        created_at: new Date().toISOString(),
      };
      ...
      const orderData = {
        store_id: 1,
        items: [{ product_id: 10, product_name: '아메리카노', quantity: 2, price: 4500 }],
        total_amount: 25000,
        payment_method: 'card',
        table_number: '3',
        is_takeout: false,
      ...
      const res = await request(app).post(`${baseUrl}`).send(orderData).expect(201);
  ...
  describe('POST /orders/:orderId/customer-token', () => {
    it('should fail if token parameter is missing', async () => {
      const res = await request(app).post(`${baseUrl}/101/customer-token`).send({}).expect(400);
    ...
    it('should update order customer_fcm_token and sync with store_customers in DB', async () => {
      ...
      expect(prisma.store_customers.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: {
            store_id: 1,
            customer_phone: 'U2FsdGVkX1+WqK4pS0+k7A==',
