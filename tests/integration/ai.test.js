...
jest.mock('../../config/prisma', () => ({
  products: {
    findActiveAndInStock: jest.fn(),
    findDessertsForStore: jest.fn(),
    findActiveByStoreId: jest.fn(),
    findByIds: jest.fn(),
  },
  orders: {
    findTrendingProducts: jest.fn(),
    findPairingData: jest.fn(),
  },
  stores: {
    findById: jest.fn(),
  },
...
describe('AI Integration Tests', () => {
  ...
  describe('POST /describe-menu', () => {
    it('should generate description successfully', async () => {
      aiService.generateMenuDescription.mockResolvedValue(
        '고소하고 깊은 맛의 프리미엄 에스프레소.'
      );
  ...
  describe('POST /recommend', () => {
    it('should recommend menus successfully', async () => {
      ...
      aiService.recommendMenus.mockResolvedValue([{ id: 1, reason: '트렌디하고 시원해서' }]);
    ...
    it('should return empty recommendations when product list is empty', async () => {
      ...
      const response = await request(app).post(`${baseUrl}/recommend`).send({ store_id: 1 });
  ...
  describe('POST /recommend-dessert', () => {
    it('should recommend desserts successfully', async () => {
      const mockDesserts = [{ id: 10, name: '마카롱', category: '디저트' }];
      ...
      expect(response.body.recommendations[0].recommend_reason).toBe(
        '달콤한 마카롱이 메인 음료와 잘 어울려요'
      );
    });
  ...
  describe('POST /storytelling', () => {
    it('should generate storytelling successfully', async () => {
      aiService.generateMenuStory.mockResolvedValue(
        '갓 볶은 원두로 정성껏 내린 풍미 가득한 스토리.'
      );
  ...
  describe('POST /analyze-menu-list', () => {
    it('should analyze menu list successfully', async () => {
      aiService.analyzeMenuList.mockResolvedValue([{ name: '아메리카노', category: '음료' }]);
    ...
    it('should return 400 when menuNames is missing', async () => {
      const response = await request(app).post(`${baseUrl}/analyze-menu-list`).send({});
  ...
  describe('POST /recommend-pairing', () => {
    it('should recommend pairing products successfully', async () => {
      Order.findPairingData.mockResolvedValue([{ product_id: 2, _count: { product_id: 15 } }]);
      Product.findByIds.mockResolvedValue([{ id: 2, name: '치즈케이크', price: 5000 }]);
