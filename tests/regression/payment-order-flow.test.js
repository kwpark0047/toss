...
describe('[회귀] 부분 환불 유효성 검사', () => {
    const mockPayment = { id: 1, payment_key: 'pk_test_abc', amount: 15000, store_id: 1, order_id: 4, method: 'CARD', status: 'DONE' };
...
describe('[회귀] Circuit Breaker 유틸리티', () => {
    ...
    test('임계치 이상 실패 → OPEN 전환', async () => {
        ...
        const fail    = () => breaker.call(async () => { throw new Error('down'); });
    ...
    test('재고 부족 주문 처리 시 서버 오류 없음', async () => {
        ...
        const res = await request(app)
            ...
            .send({
                ...
                total_amount: 5000,
                phone: '01055556666',
                items: [{ product_id: 10, product_name: '한정 메뉴', price: 5000, quantity: 1, subtotal: 5000 }]
            });
